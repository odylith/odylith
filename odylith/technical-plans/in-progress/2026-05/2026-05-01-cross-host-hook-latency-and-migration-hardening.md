...
- [x] `PYTHONPATH=src .venv/bin/python -m odylith.cli casebook validate --repo-root .`, `validate backlog-contract`, `validate plan-workstream-binding`, `validate plan-risk-mitigation`, `validate discipline`, `validate guidance-behavior`, and `release migration-gate --target-version 0.1.13 --json` all passed after the engine-integrity hardening and migration-observer marker updates.
