...
## Goal
Make Odylith Discipline the v0.1.11 platform layer that changes how an
...
- Voice and intervention rendering must stay live and evidence-shaped. Odylith Discipline
...
- Unsupported host families or execution lanes are fail-closed hard-law inputs, not advisory metadata. Odylith Discipline may normalize known aliases, but an unknown host or lane must defer to a supported Codex/Claude and dev/dogfood/consumer contract before admitting work.
...
- Hot discipline checks are Tier 0/Tier 1 local computation only: no provider calls, no host model calls, no subagent spawn for classification, no broad scan, no projection-store expansion, no full validation, and no benchmark execution.
...
- Guidance Behavior and Odylith Discipline benchmark scenarios are
...
- Odylith Discipline signal extraction distinguishes unsafe moves from operator
...
- Related bug search: no stronger existing Casebook bug was found for the Odylith Discipline layer itself before this plan was created.

## Contracts
- `odylith_discipline.v1`
- `odylith_discipline_learning.v1`
- `odylith_discipline_runtime_budget.v1`
- `odylith_discipline_host_lane_support.v1`
- `discipline_evaluation_corpus.v1`
- `odylith_discipline_validation.v1`
- Compact practice events under `odylith_discipline_learning.v1`
...
- `odylith discipline check --repo-root . --intent-file PATH [--host HOST] [--lane dev|dogfood|consumer] [--json]`
- `odylith discipline explain --repo-root . --decision-id ID [--json]`
- `odylith validate discipline --repo-root . [--case-id ID...] [--json]`
- `odylith benchmark --profile quick --family discipline --no-write-report --json`

## Success Criteria
- The discipline validator is deterministic, credit-free, and green for seeded cases.
- Guidance Behavior remains green as the first deterministic pressure-family lane.
- The quick `discipline` benchmark family selects only that family and reports hard-law, stance, learning, latency, and credit metrics.
...
- Codex and Claude expose the same Odylith Discipline contract across dev, dogfood, and consumer lanes.
- Codex and Claude host model aliases normalize to adapter families; Odylith Discipline
...
- `odylith discipline check` records a local `.odylith/cache` decision receipt,
  and `odylith discipline explain` explains that receipt without model calls,
...
## Validation Plan
- `PYTHONPATH=src python -m pytest -q tests/unit/runtime/test_discipline.py`
- `PYTHONPATH=src python -m pytest -q tests/unit/runtime/test_validate_discipline.py`
...
- `./.odylith/bin/odylith validate discipline --repo-root .`
...
- `PYTHONPATH=src python -m odylith.cli discipline status --repo-root . --json`: passed; reported Codex and Claude across dev, dogfood, and consumer lanes, with `provider_calls=0`, `host_model_calls=0`, `broad_scans=0`, `full_validations=0`, and `projection_expansions=0`.
- `PYTHONPATH=src python -m odylith.cli validate discipline --repo-root . --json`: passed, 19 cases, 0 issues, 8 host/lane parity rows, `discipline_hard_law_pass_rate=1.0`, `discipline_hot_path_budget_pass_rate=1.0`, `discipline_provider_call_count=0`, `discipline_host_model_call_count=0`, `discipline_false_allow_rate=0.0`, `discipline_false_block_rate=0.0`.
...
- `./.odylith/bin/odylith validate discipline --repo-root . --json`: passed after hardening, 19 cases, 0 issues, platform integration passed across Context, Execution, Memory, Intervention, and Benchmark; proof obligation, tool affordance, memory recurrence, learning replay, and intervention visibility metrics all reported `1.0`; hot-path host model and provider call counts stayed `0`.
- `./.odylith/bin/odylith validate guidance-behavior --repo-root .`: passed after hardening, 6 cases and 11 guidance checks.
- `./.odylith/bin/odylith benchmark --repo-root . --profile quick --family discipline --no-write-report --json`: provisional pass after hardening, 7 selected scenarios, hard quality gate cleared, fairness contract passed, 0 hard-gate failures; advisory widening-rate check remains nonblocking mechanism attention and is not release/public proof.
- Superseded on 2026-04-18 by B-110-tagged benchmark scenarios: quick `discipline` now clears hard, secondary, and advisory checks with `odylith_requires_widening_rate=0.0`.
...
- 2026-04-18 hardening pass:
  ...
  - `PYTHONPATH=src python -m py_compile src/odylith/runtime/discipline/*.py src/odylith/runtime/governance/validate_discipline.py src/odylith/runtime/intervention_engine/alignment_evidence.py src/odylith/runtime/execution_engine/runtime_surface_governance.py src/odylith/cli.py`: passed.
  - `PYTHONPATH=src pytest -q tests/unit/runtime/test_execution_engine.py tests/unit/runtime/test_execution_engine_handshake.py tests/unit/runtime/test_tooling_context_packet_builder.py tests/unit/runtime/test_tooling_memory_contracts.py tests/unit/runtime/test_odylith_benchmark_corpus.py tests/unit/runtime/test_odylith_benchmark_runner.py tests/unit/runtime/test_intervention_engine.py tests/unit/runtime/test_intervention_alignment_evidence.py tests/unit/test_cli.py tests/unit/test_cli_audit.py -k 'discipline or guidance_behavior or execution_engine or intervention or context_packet or memory'`: 141 passed, 427 deselected.
  - `./.odylith/bin/odylith benchmark --repo-root . --profile quick --family discipline --no-write-report --json`: superseded by the latest B-110-tagged quick run where advisory checks clear and `odylith_requires_widening_rate=0.0`.
...
- 2026-04-18 final post-sync QA:
  ...
  - `./.odylith/bin/odylith validate discipline --repo-root . --json`: passed after sync; 19 cases, platform integration passed, all declared expectation checks matched, `discipline_provider_call_count=0`, `discipline_host_model_call_count=0`, and hot-path budget pass rate 1.0.
  ...
  - `./.odylith/bin/odylith benchmark --repo-root . --profile quick --family discipline --no-write-report --json`: `report_id=0d3192435b70c663`, provisional pass, 7 scenarios, hard quality gate cleared, secondary guardrails cleared, fairness passed; superseded by `report_id=2fca44b29de6d155` with advisory checks clear.
...
- 2026-04-18 aggressive QA follow-up:
  ...
  - `PYTHONPATH=src pytest -q tests/unit/runtime/test_discipline.py tests/unit/runtime/test_validate_discipline.py tests/unit/runtime/test_sync_cli_compat.py -k 'discipline or run_callable_with_heartbeat or dashboard_refresh or selective'`: 61 passed, 35 deselected.
  - `PYTHONPATH=src python -m py_compile src/odylith/runtime/discipline/*.py src/odylith/runtime/governance/validate_discipline.py src/odylith/runtime/governance/sync_workstream_artifacts.py`: passed.
  - `./.odylith/bin/odylith validate discipline --repo-root . --json`: passed; 19 cases, platform integration passed, hot-path provider calls 0, hot-path host model calls 0, hot-path budget pass rate 1.0.
...
- 2026-04-18 adaptive materialization hardening:
  - Centralized pressure/law signal extraction in
    `src/odylith/runtime/discipline/signals.py` and moved hard-law recovery
  ...
  - Tagged all `guidance_behavior` and `discipline` quick
  ...
  - `PYTHONPATH=src pytest -q tests/unit/runtime/test_discipline.py tests/unit/runtime/test_validate_discipline.py`: 41 passed for non-rigid behavior, learning-feedback affordances, hard-law policy coverage, and ephemeral source-ref filtering.
  - `PYTHONPATH=src pytest -q tests/unit/runtime`: 2113 passed.
  - `PYTHONPATH=src pytest -q tests/unit/runtime/test_odylith_benchmark_corpus.py tests/unit/runtime/test_discipline.py tests/unit/runtime/test_validate_discipline.py tests/unit/runtime/test_validate_guidance_behavior.py`: 80 passed after B-110 corpus tagging.
  - `./.odylith/bin/odylith validate discipline --repo-root .`: passed.
  - `./.odylith/bin/odylith validate guidance-behavior --repo-root .`: passed.
  - `./.odylith/bin/odylith benchmark --profile quick --family discipline --no-write-report --json`: `report_id=2fca44b29de6d155`, provisional pass, 7 scenarios, hard quality gate cleared, secondary guardrails cleared, advisory checks cleared, fairness passed, `odylith_requires_widening_rate=0.0`.
...
- 2026-04-18 post-sync proof:
  ...
  - `./.odylith/bin/odylith validate discipline --repo-root .`: passed after sync.
  - `./.odylith/bin/odylith validate guidance-behavior --repo-root .`: passed after sync.
  - `./.odylith/bin/odylith benchmark --profile quick --family discipline --no-write-report --json`: `report_id=db72f40e8035386e`, provisional pass, 7 scenarios, hard quality gate cleared, secondary guardrails cleared, advisory checks cleared, fairness passed, `odylith_requires_widening_rate=0.0`.
...
- 2026-04-18 aggressive false-block and credit-safety hardening:
  ...
  - Added five Odylith Discipline corpus cases for allowed technical
  ...
  - Hardened benchmark metric summaries to publish false allow/block,
    ...
    Odylith Discipline case results.
  - `PYTHONPATH=src pytest -q tests/unit/runtime/test_discipline.py tests/unit/runtime/test_validate_discipline.py`: 48 passed.
  ...
  - `./.odylith/bin/odylith validate discipline --repo-root .`: passed, 24 cases, zero host/provider calls.
  - `./.odylith/bin/odylith validate guidance-behavior --repo-root .`: passed.
  - `./.odylith/bin/odylith benchmark --profile quick --family discipline --no-write-report --json`: `report_id=1be88ce4ead87770`, provisional pass, 7 scenarios, hard quality gate cleared, secondary guardrails cleared, advisory checks cleared, fairness passed, `odylith_requires_widening_rate=0.0`.
...
- 2026-04-18 post-refresh QA:
  ...
  - `./.odylith/bin/odylith validate discipline --repo-root .`: passed.
  - `./.odylith/bin/odylith validate guidance-behavior --repo-root .`: passed.
  - `PYTHONPATH=src pytest -q tests/unit/runtime/test_discipline.py tests/unit/runtime/test_validate_discipline.py tests/unit/runtime/test_validate_guidance_behavior.py tests/unit/runtime/test_odylith_benchmark_corpus.py tests/unit/runtime/test_odylith_benchmark_runner.py`: 273 passed.
  ...
  - Post-refresh quick `discipline`: `report_id=e54e6fe7c7f3b52b`, provisional pass, hard/secondary/advisory gates true, `odylith_requires_widening_rate=0.0`, validation delta `0.714`, critical validation delta `0.714`.
...
- Added `src/odylith/runtime/discipline/` as the shared adaptive, zero-credit runtime package instead of scattering checks through host adapters.
- Added `odylith/runtime/source/discipline-evaluation-corpus.v1.json` and bundle mirror under `src/odylith/bundle/assets/odylith/runtime/source/`.
- Added `odylith validate discipline` and `odylith discipline status/check/explain`.
- Kept `odylith validate guidance-behavior` intact and made it the first deterministic pressure-family validator under Odylith Discipline.
- Added `discipline` benchmark family, taxonomy/rules integration, quick local-only selection, and source/bundle benchmark corpus entries.
- Added `odylith_discipline_host_lane_support.v1` and explicit
...
- Added `src/odylith/runtime/discipline/memory.py` for compact, sanitized
...
- Added focused proof and voice helpers so proof obligations are reusable and
  intervention candidates stay evidence-shaped: Odylith Discipline now emits explicit
...
- Odylith Discipline validation now checks declared corpus obligations instead of only
  ...
  and Benchmark.
- Intervention alignment now treats `discipline_contract` as a
  first-class evidence class. Passing Odylith Discipline summaries stay quiet; failing
  summaries become a single high-signal invariant fact instead of chat noise.
- `odylith discipline check` now records local decision receipts under
  `.odylith/cache/discipline/decisions`, and
  `odylith discipline explain` turns those receipts into a concise evidence,
  recovery, learning, and budget explanation.
- `odylith discipline status` now reports runtime summary, active hard laws,
...
- Hardened compact memory and validator handling for transcript-shaped refs and
  ...
  counter values.
- Refactored proof obligations into `src/odylith/runtime/discipline/proof.py`
  so hard-law failures produce reusable, canonical obligations instead of
  scattered prose strings.
- Refactored Odylith Discipline intervention shaping into
  `src/odylith/runtime/discipline/voice.py`. Odylith Discipline emits evidence,
...
- Expanded deterministic Odylith Discipline validation to enforce corpus-declared
...
- Added platform integration checks for Context, Execution, Memory,
  Intervention, and Benchmark so Odylith Discipline cannot pass as an
  isolated corpus validator while the rest of Odylith is disconnected.
- Integrated Odylith Discipline evidence into Execution Engine runtime-surface
...
- Hardened the Compass browser-test narrator fixture so evolving governed
  ...
  stagey voice.
- Made Odylith Discipline decision ids deterministic for evidence maps whose keys collide
