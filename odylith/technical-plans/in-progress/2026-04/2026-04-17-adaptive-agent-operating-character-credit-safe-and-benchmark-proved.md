...
- `PYTHONPATH=src python -m py_compile src/odylith/runtime/character/*.py src/odylith/runtime/governance/validate_agent_operating_character.py src/odylith/cli.py`: passed after the learning-spine and `discipline explain` implementation.
...
- `PYTHONPATH=src python -m odylith.cli discipline explain --repo-root . --decision-id character:claude:dogfood:980a0af875f63e4a --json`: passed from the local decision receipt; returned `explained`, `block`, `fresh_proof_completion`, and host model calls 0.
...
- Added `odylith validate agent-operating-character` and `odylith discipline status/check/explain`.
...
- Refactored Odylith Discipline intervention shaping into
  `src/odylith/runtime/character/voice.py`. Odylith Discipline emits evidence,
