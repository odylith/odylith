...
- [x] Cheap suppression is not allowed to feel fake. If the ladder keeps
      Compass off a fresh provider pass, the scoped brief still has to stay on
      exact-cache replay or explicit `unavailable` instead of sliding back
      into cheaper synthetic prose.
