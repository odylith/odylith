...
superseded. The scoped-readiness expansion is removed and the unrun three-role
experiment stays paused. Source fidelity, safety and 90/120/150 remain fixed.

The clean local candidate build at 6e8cb1903 completed in 262.773 seconds with 1,447 declared inputs
unchanged; clean installation completed in 30.883 seconds with memory installed,
...
residents. Independent adjudication identifies that denial as a false negative.
The bounded correction changes one existing shared state-object role description
used by the author schema and reviewer: a person may qualify when the cited source
changes or reviews that person's state; a performer-only citation remains invalid.
Author prompt v57 identifies the correction; v54--v56 remain rejected. Reviewer
receipt v2, schema, calls, budgets and other mechanisms are unchanged.

Three frozen, read-only Astra-medium reviewer controls passed within their
30-second diagnostic bounds: exact candidate (6.188 seconds), heading-equivalent
source (7.248 seconds), and the performer-as-state negative (6.015 seconds).
They are unsealed reviewer evidence only, not fresh authoring, package,
...
regression test was excluded; use the resolved existing Casebook record instead.
