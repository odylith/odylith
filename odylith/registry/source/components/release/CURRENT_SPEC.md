...
- 2026-06-27: Added default installed greenfield CLI auto-rescue proof to the local release matrix so post-confirm repair changes must prove packaged-runtime `--repair-tier auto` escalation, 90 second rescue-budget custody, final-manifest pass status, governed writes, and repaired semantic issue-code evidence before release-readiness can be claimed. A follow-up installed proof found and fixed the harness env-custody bug: standard cases no longer receive the internal probe token, while rescue smoke does. Final dist `odylith-local-release-0.1.15-f6a06af6` passed five standard installed greenfield cases in 20.223-22.425s with hard 10/10 scores and passed installed CLI auto-rescue smoke in 26.842s, all with zero issues and complete governed artifacts. `RESCUE_SMOKE=0` is debug-only and not release proof. (Plan: [B-142](odylith/radar/radar.html?view=plan&workstream=B-142); Bugs: `CB-208`, `CB-209`)
