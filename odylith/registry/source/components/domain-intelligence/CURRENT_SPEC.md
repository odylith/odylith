...
- `greenfield_post_confirm_completion.py` owns package assembly and report
  ...
  in `greenfield_post_confirm_patchset.py`. Repair callbacks must read
  `PatchSet` target layers through `greenfield_post_confirm_repair_context.py`
  instead of reinterpreting review prose.
