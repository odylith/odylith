...
- **2026-06-29 · Implementation:** Canonical projection repetition custody now keeps typed semantic fact provenance through package-quality scoring. `CanonicalProjectionFact` variants retain source layer, semantic node, source path, and sanctioned projection IDs; the repetition gate admits repeated copy only when the artifact projection is allowed for that fact. Release-matrix failure evidence now persists bounded create stdout/stderr/blocker excerpts only for failed cases, keeps passed-case failure evidence empty, suppresses local-server disconnect noise, and strictly verifies matrix-root cleanup. Fresh installed matrix proof passed thirteen high-variance standard cases at hard 10/10 under 60s, including quantum and sparse-intent replays, with browser proof passing for all cases and auto-rescue wiring smoke passing in 35.012s.
  - Scope: B-142
  - Evidence: src/odylith/runtime/domain_intelligence/greenfield_canonical_projection_facts.py, src/odylith/runtime/artifact_quality/greenfield_package_quality.py +4 more
