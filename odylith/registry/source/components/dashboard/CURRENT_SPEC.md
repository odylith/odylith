...
## Feature History
- 2026-06-29: Tightened generated surface asset custody for greenfield-created repos. `brand_assets.py` now owns seeding missing managed `odylith/surfaces/brand` assets from packaged product assets before greenfield dashboard refresh, and the greenfield rollback guard snapshots that managed brand tree so failed post-confirm writes do not leave partial browser dependencies. Static surface-health proof now checks local HTML asset references, including lockup images, instead of waiting for browser proof to discover 404s. The retained wildfire mutual-aid source-local replay seeded 22 brand assets, passed generated browser proof with zero issues, and scored 10/10 after previously failing on missing lockup requests. (Plan: [B-142](odylith/radar/radar.html?view=plan&workstream=B-142); Bug: `CB-209`)
