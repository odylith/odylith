...
- Rendering internal diagnostic records as product chrome.
...
- The shell must not render internal diagnostic UI. Do not add a shell status
  drawer, status presenter, recorder tape, cockpit grid, chart canvas, ECharts
  dependency, or snapshot-style status slab. Internal diagnostics may remain in
  explicit debug artifacts, but the top-level dashboard shell must not load,
  embed, or render internal delivery,
...
externalizing the bundle. It must not derive or render shell diagnostic payloads.
...
also must not replace that dedupe rule with internal status cockpit chrome.
...
- When a child surface already explains its own stale-runtime or freshness
  ...
  presenter; wrapper-level notices must be explicit, narrow, and not diagnostic
  cockpit UI.
...
- **2026-09-25 · Implementation:** Implementation evidence linked this component to governed work with workstream scope preserved; 2 verifiable artifact references.
  - Scope: B-142
  - Evidence: `odylith/registry/source/components/release/CURRENT_SPEC.md`, `tests/unit/install/test_greenfield_browser_surface_proof.py`
- **2026-09-25 · Implementation:** Implementation evidence linked this component to governed work with workstream scope preserved; 2 verifiable artifact references.
  - Scope: B-142
...
- **2026-09-24 · Implementation:** Implementation evidence linked this component to governed work with workstream scope preserved; 2 verifiable artifact references.
  - Scope: B-142
...
- 2026-06-30: Added release-proof custody for generated shell payload readback. (Plan: [B-142](odylith/radar/radar.html?view=plan&workstream=B-142); Bug: `CB-209`; Diagram: D-046)
  ...
  must come from the generated tooling payload, and Radar, Registry, Atlas,
  Compass, Casebook, and tooling-shell payloads must parse with surface-owned
  content before browser proof can contribute to a premium score.
- 2026-06-29: Tightened generated surface asset custody for greenfield-created repos. `brand_assets.py` now owns seeding missing managed `odylith/surfaces/brand` assets from packaged product assets before greenfield dashboard refresh, and the greenfield rollback guard snapshots that managed brand tree so failed post-confirm writes do not leave partial browser dependencies. Static surface-health proof now checks local HTML asset references, including lockup images, instead of waiting for browser proof to discover 404s. The retained source-local surface replay seeded 22 brand assets, passed generated browser proof with zero issues, and scored 10/10 after previously failing on missing lockup requests. (Plan: [B-142](odylith/radar/radar.html?view=plan&workstream=B-142); Bug: `CB-209`)
...
- 2026-04-16: Deleted the shell status presenter, ECharts dependency, recorder/chart CSS, and internal status render path after CB-120 proved diagnostic cockpit chrome was leaking into dashboard product surfaces. Dashboard now treats any cockpit, recorder, chart, or status slab in the shell DOM as a regression. (Plan: [B-025](odylith/radar/radar.html?view=plan&workstream=B-025); Bug: `CB-120`)
