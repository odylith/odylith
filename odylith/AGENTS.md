...
- In coding-agent commentary, keep startup, fallback, routing, and packet-selection internals implicit. Describe progress in task terms like the exact file/workstream, the bug under test, or the validation in flight. If startup needs narrowing but work can continue, do not narrate it, and never say `Startup fell back`. Do not surface routine `odylith start`, `odylith context`, or `odylith query` commands in progress updates, and never prefix commentary with control-plane receipt labels. Keep normal commentary task-first and human. Reserve `Odylith Insight:`, `Odylith History:`, or `Odylith Risks:` for rare high-signal moments.
...
- Empty or thin consumer repos can still receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt "<request>"` for the no-write Product Intent Confirmation request, then write a compact product-first interpretation in chat from live reasoning as sectioned Markdown: title, Product story, State object, First complete path, Human actors, External systems, Internal product systems, Critical assumptions, Ambiguities, Proof boundary, and `## Choose one command`. The command block must show `Start your reply with exactly one command: **CONFIRM**, **EDIT**, or **REJECT**.`, then sections: **Command: `CONFIRM`**, **Command: `EDIT`**, and **Command: `REJECT`** plus `Reply starts with: CONFIRM`, `Reply starts with: EDIT`, and `Reply starts with: REJECT`. Use short paragraphs for story/path/proof and bullets for actors, systems, assumptions, and ambiguities; do not collapse the confirmation into a wall of prose or wrap normal domain nouns in code ticks or decorative bold markers. Do not generate backlog, Registry, Atlas, release waves, validation obligations, or create writes before intent confirmation; staging is the only allowed pre-confirm runtime artifact. After confirmation, write that same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield compile-transaction --repo-root . --prompt "<request>" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --output .odylith/runtime/greenfield/product-create-transaction.v1.json --release 0.0.1`; Odylith may normalize that Markdown into `.odylith/runtime/greenfield/confirmed-intent.json`, then hashes gated ProductCreateTransaction before records can be confirmed. The first **CONFIRM** compiles and hash-gates the transaction before any governed records are written. The hash confirmation runs `odylith greenfield create --repo-root . --transaction-file .odylith/runtime/greenfield/product-create-transaction.v1.json --transaction-hash <hash> --confirm`; confirmed create only verifies the hash, writes atomically, validates readback, refreshes readable views, and reports success or environment/IO failure. Treat **EDIT** corrections after command as evidence and rebuild. Treat **REJECT** as stop with no governed records. Do not ask the operator to inspect proposal JSON or confirm a second time around uncompiled Markdown. Do not inspect Odylith source files, Python modules, `.odylith`, local examples, or bundle assets to discover schema fields, and do not hand-author, switch to, or repair proposal JSON after confirmation. Do not narrate parser/schema retries or intermediate transaction-compile failures in operator chat; wait for the final transaction/result and keep the user-visible transcript to neutral progress, the human-readable created-record summary, or final blockers. Do not run confirmed create from a thin prompt without a verified ProductCreateTransaction. No canned domain families, scaffolds, or code before product gates.
...
- Governance-learning is mandatory across maintainer, pinned dogfood, detached `source-local`, installed consumer repos, Codex, Claude Code, and bundled/generated guidance. Every durable error, escaped defect, failed mechanism, failed simulation, live repro, bad generated artifact, semantic drift, quality-gate failure, latency breach, architecture decision, validation result, or release-risk learning must update the correct governed surface before the agent continues, commits, builds, releases, or claims completion: Casebook for defects and regressions, Radar or technical plans for planned work, Registry for component-contract changes, Atlas for changed flow/topology/projection boundaries, and Compass for execution decisions and proof checkpoints. Before fixing any bug, search existing Casebook and related governance artifacts, read prior failed mechanisms, failed fix attempts, and guardrails, and do not repeat a fix path that has already failed. Search existing truth first, avoid duplicate Casebook bugs, use the Odylith CLI where it exists, and do not treat chat memory or final-answer summaries as durable governance truth.
