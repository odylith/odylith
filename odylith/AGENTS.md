...
- Empty or thin consumer repos can still receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt "<request>"` for the no-write Product Intent Confirmation request, then write a compact product-first interpretation in chat from live reasoning: product story, actors, systems, assumptions, ambiguities, and a clear Next step block with Confirm, Edit, and Reject choices. Do not generate backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or prompt-only `greenfield create` records before intent confirmation. After confirmation, use `odylith greenfield propose --confirm-intent --format json` internally for the complete schema/proof contract rather than searching Odylith source files, Python modules, `.odylith`, local examples, or bundle assets. Do not show that contract as a second approval step. Author an internal proposal payload from the confirmed product shape, then run `odylith greenfield apply --repo-root . --proposal-file .odylith/runtime/greenfield/active-proposal.v1.json --confirm --release 0.0.1`; apply validates it, runs the Tribunal write gate, writes records, and refreshes readable views from the same confirmation. Do not ask the operator to inspect proposal JSON or confirm a second time unless they explicitly request a review artifact. No canned domain families, scaffolds, or code before product gates. Surface only the human-readable created-record summary or validation/Tribunal blockers.
