...
- Greenfield uses `odylith greenfield propose --repo-root . --prompt "<request>"`. Input and corrections are untrusted evidence. Odylith builds typed custody facts and quality-gates the full staged ProductCreateTransaction, then shows a read-only preview. No qualified confirmation interface is attached to that preview. Do not append chat decision commands, offer publication, or run create from a chat approval. Host names and registered hooks do not prove qualification. Explicit operator invocation of `odylith greenfield create` with `--transaction-file`, `--transaction-hash`, and `--confirm` remains separate: it verifies receipt, hash and preconditions, writes sealed bytes under rollback guard, and validates readback without model reasoning, generation or repair. Ask one focused question only for material uncertainty; show other gaps as assumptions. Markdown is a view, never product truth. Do not hand-author proposal JSON, inspect source for schema, or narrate parser/schema retries. Show the preview and its material confirmation blocker plainly.
