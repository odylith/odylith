...
- Empty or thin consumer repos can still receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt "<request>"` to review a compact product-first interpretation, direction choices, non-goals, first release ambition, and proof gates. On confirmation, use `odylith greenfield create --repo-root . --prompt "<request>" --release 0.0.1 --confirm` so Odylith validates the full proposal, runs the Tribunal write gate, writes governed records, refreshes readable views, and hands off without exposing tool internals as product requirements. Do not hand-author proposal JSON, reconstruct it from prose, or start coding before the product gates are accepted.
