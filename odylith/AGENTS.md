...
- Empty or thin consumer repos can still receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt "<request>"` for the no-write Product Intent Confirmation request, then write a compact product-first interpretation in chat from live reasoning as sectioned Markdown: title, Product story, State object, First complete path, Human actors, External systems, Internal product systems, Critical assumptions, Ambiguities, Proof boundary, and a clear Next step block with Confirm, Edit, and Reject as three separate bullet lines. Use short paragraphs for story/path/proof and bullets for actors, systems, assumptions, ambiguities, and the Next step choices; do not collapse the confirmation into a wall of prose or wrap normal domain nouns in code ticks or decorative bold markers. Do not generate backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or create writes before intent confirmation. After confirmation, write that same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt "<request>" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes that Markdown into `.odylith/runtime/greenfield/confirmed-intent.json`, builds the governed proposal from the accepted narrative, validates it, runs the bounded, provider-free post-confirm repair loop, runs the Tribunal write gate, writes records only after the final manifest passes, and refreshes readable views. `odylith greenfield propose --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm-intent --format json` is only an optional audit artifact when explicitly requested; the normal write path remains confirmed create from the accepted intent file. Do not ask the operator to inspect proposal JSON or confirm a second time by default. Do not inspect Odylith source files, Python modules, `.odylith`, local examples, or bundle assets to discover schema fields, and do not hand-author, switch to, or repair proposal JSON after confirmation. Do not narrate parser/schema retries or intermediate create-shape failures in operator chat; wait for the final manifest/result and keep the user-visible transcript to neutral progress, the human-readable created-record summary, or the final validation/Tribunal blockers. Do not stop after an intermediate repairable quality issue. Do not run confirmed create from a thin prompt without `--intent-file`. No canned domain families, scaffolds, or code before product gates.
