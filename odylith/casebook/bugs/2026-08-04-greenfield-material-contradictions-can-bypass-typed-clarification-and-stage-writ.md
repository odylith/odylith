...
- Current-Contract Protected Holdout Reopen (2026-09-03): The independently authored final-holdout v4 case GFH19-014 reproduced the pre-materialization boundary failure against immutable dist-v14. Greenfield returned `mode=product_create_transaction`, staged a pending transaction, and wrote candidate plus prompt/edit runtime evidence instead of returning one focused material clarification. Three neighboring clarification cases did not prove governed writes; their failures were write-observer overcount owned by CB-329. This isolates one genuine product recurrence: every authoring route must pass one pure typed clarification decision before any candidate, pending transaction, subprocess, or prompt-side persistence. The disclosed case is retired to regression and must pass with zero governed or Greenfield staging writes before another blind holdout is commissioned.

- Direct-Graph Source Resolution (2026-09-03): Intent-authoring v20 makes
  material ambiguity a closed, source-cited result of the same single authored
  graph. It returns the existing typed clarification sentinel before graph
  derivation or any candidate, transaction, subprocess, or prompt-side write;
  clarification carries no terminal result. No second classifier, regex rule,
  phrase list, retry, or response repair was added. Focused and combined source
  proof passes, including governed-write audit controls. CB-315 remains in
  progress until the immutable installed public matrix and new blind holdout
  prove the boundary end to end.
