...
- Disclosed Replacement False-Clarification Closure (2026-08-16): Materiality now treats settled corrections, bounded non-material assumptions, and presentation-only labels as non-blocking while preserving source-grounded workflow, state, and proof custody. The three retired expected-commit controls compile, and all seven true clarification controls still stop before writes with their focused fields. The combined envelope, clarification, and materiality pack passed `68/68`; the final runtime matrix passed `396/396`; independent closure found no P0/P1. CB-319 remains `InProgress` until the rebuilt installed candidate and a new untouched holdout prove the distinction outside the disclosed corpus.
