...
- Follow-Up Evidence (2026-05-08 / v0.1.15 local greenfield): An external-domain greenfield apply produced component specs whose visible Registry detail still read like a template: `Experience Boundary is a application component registered through odylith component register`, repeated the same planned-ownership structure across components, leaked raw risk metadata such as `R1.` / `odylith_assumption`, and used generic interface/dependency/proof language instead of component-specific ownership and proof contracts.
...
- Follow-Up Solution (2026-05-08): Extract Registry spec rendering into `component_spec_rendering`, render kind-specific component dossiers (application interaction, runtime contract, tooling proof harness), infer deterministic domain component profiles for generic greenfield prompts, add domain-profiled component generation, and normalize inherited risk/security/compliance posture without leaking raw proposal metadata.
...
- Regression Tests Added: tests/unit/runtime/test_greenfield_proposals.py covers child topology metadata, Atlas related_backlog links, richer Registry specs, shallow proposal rejection, and domain-profiled component specs that reject generic Experience Boundary/template text and raw risk metadata. tests/unit/runtime/test_component_authoring.py covers responsibility-backed component specs and the extracted Registry spec renderer.
