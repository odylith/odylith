...
- Detected By: Maintainer transcript review of a domain-bearing greenfield proposal preview.
...
- Root Cause: The generic and robot-swarm diagram builders used f'{title} <view name>' for every diagram title.
...
- Verification: Domain-bearing preview now renders Architecture review views as System Overview, First Slice Flow, Component Ownership Map, Domain State Model, and Validation And Release Topology without the prompt prefix. Focused generic, robot-swarm, and compact-preview regression tests pass in source-local mode.

- Prevention: Regression tests assert concise diagram titles for generic and robot-swarm proposals, plus a Tribunal rejection for project-title-prefixed diagram titles.
