...
- Regression Tests Added: `tests/unit/install/test_release_bootstrap.py` covers hosted install shell suppression for classic wrapped, Rich wrapped, and unindented trusted-root continuations while preserving unexpected verifier warnings. `tests/unit/install/test_release_assets.py` covers managed verifier suppression for the same classes plus release download metadata-only reporting with clean stdout/stderr.
