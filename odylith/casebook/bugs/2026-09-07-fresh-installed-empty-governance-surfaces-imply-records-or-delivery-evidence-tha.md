...
- Explicit Observation Readiness (2026-09-17): The route callback now acknowledges its recorded count through a test-only browser marker; the test waits for that exact receipt and still asserts the Python count before releasing each delayed response. The entrypoint check waits on its exact selected-tab locator rather than reading immediately after URL redirection. Both use the existing 15-second browser bound, with no sleeps or production change. All four delayed-response cases and the five-surface entrypoint control pass (5 tests, 9.53 seconds). This corrects observation timing without waiving the stale-response invariant; broader frozen proof remains required at /Users/freedom/.codex/odylith-scoped-readiness.wfD6SX/checkpoint-2/.

- Browser Observation Race (2026-09-17): The scoped-readiness frozen run preserves all inputs but the Radar desktop delayed-response control observes its third request before the route callback appends it: pending count is two rather than three. The test stops before its stale-response assertions; this is not evidence that the product accepted a stale response. A separate direct-entrypoint smoke control reads aria-selected=false after URL redirection but before shell activation. Correct only those observation boundaries with explicit callback and selected-state readiness; retain all real delayed responses, stale-detail, heading and clean-page assertions. No fixed sleeps, weakened counts, enlarged deadlines or production navigation change is justified. The failed receipts remain under /Users/freedom/.codex/odylith-scoped-readiness.wfD6SX/checkpoint/; independent adjudication and a fresh frozen run are required.
...
- Registry Reading Follow-up (2026-09-07): The residual mobile Diagnostics overlap is corrected in source by Registry-only normal-flow filters at the existing compact breakpoint. The separate clipped-spec defect moves disclosure/prose/table layout into registry_spec_reading_ui.py with shrinkable grids and wrapped prose while preserving table-local scrolling. Ten red controls and two baseline passes precede the fix; final focused proof passes 20 browser and 41 unit checks, independent screenshots agree, and the frozen broader surface suite passes 270 checks. A missing refresh-cache dependency for the extracted owner is reproduced separately and must be closed before commit. No shared sticky primitive or dashboard header changes. The 4ca2f3d5 installed evidence remains baseline; this does not establish a fresh installed fix, comprehensive accessibility, semantic quality or consumer SLA. Evidence: /private/tmp/odylith-registry-reading-proof.Lw8Pja/.

- Registry Reading Cache Closure (2026-09-07): The extracted source owner now invalidates Registry surface-refresh reuse; the actual-source red reproduction is retained and all 16 cache/owned-refresh controls pass. This closes the pre-commit dependency obligation in the reading follow-up. Layout bytes, shared primitive and header are unchanged by this integration step. Evidence: /private/tmp/odylith-registry-reading-proof.Lw8Pja/cache-green.xml.
