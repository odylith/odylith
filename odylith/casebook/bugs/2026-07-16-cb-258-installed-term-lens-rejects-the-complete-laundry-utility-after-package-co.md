...
- Final Holdout Reopen (2026-08-04): Eight committed cases in exact-revision holdout `7f03f7cf8` preserved only three of the required four or five domain terms. In every case, the omitted terms were the named external sources accepted in operator evidence, including `Grove Roster`, `Hall Calendar`, `Tool Shelf Index`, `Tide Ledger`, `Playbill File`, `Rack Map`, `Path Gazette`, `Steward Rota`, `Clay Ledger`, and `Roost Index`.

- Follow-Up Root Cause (2026-08-04): Operator evidence carried explicit source relations, but the generic intent hypothesis initialized an empty external boundary and prompt canonicalization later retained only the first-path sentence. First-path-only fallback could not recover the discarded source names, so all downstream surfaces were internally consistent while losing accepted evidence. A first repair extracted sources during generic normalization and checked all rendered text; adversarial review rejected it because it could infer actors or generic dependencies as systems, override an explicit structured empty boundary, and pass when a source appeared only in Radar copy.

- Follow-Up Solution (2026-08-04): Extract only explicit named source relations while building the untrusted operator-evidence hypothesis, before prompt compaction. Later confirmed-intent normalization no longer lets raw prompt prose override an explicit structured boundary; first-path semantic inference remains separately governed. Narrative extraction rejects generic people, interview, requirement, and dependency prose while retaining named providers and source carriers across domains. The architect lens now requires each accepted external-system label on Atlas, Registry, or component-spec architecture output; mentions in Radar, briefs, or release prose cannot satisfy it.

- Follow-Up Verification (2026-08-04): Positive and negative source-custody regressions cover carrier names, single-name providers, structured source fields, trailing action prose, actor/product false positives, generic interview provenance, and generic JSON dependencies. Full prewrite regressions preserve disclosed named sources through rendered Atlas and Registry evidence. Focused source, quality-lens, clipping, and Project tests pass 75 tests, and clean installed holdout proof remains required before changing status.

- Final Holdout Oracle Finding (2026-08-05): Independent adjudication found that all 24 ba25 holdout cases required at least one product term that the same case also forbade as leakage. That cluster was an unsatisfiable evaluator contract, not evidence that product-domain custody should be relaxed. Frozen preflight now rejects normalized required/forbidden overlap. Separately, the real product miss on the acoustic-transect case was repaired by recovering generic actor-action-system-output evidence before clarification; the retired 24-case package replay is clean. A new disjoint, independently reviewed holdout remains required.
