...
- Current Distribution Preparation (2026-09-10): A committed-source export of 9786fd05, excluding the retired holdout test before archive traversal, separates build inputs from the preserved recovery-required workspace. The canonical component-forensics prerequisite identifies only Dashboard and Radar sidecars as stale. Their owning CLI regenerates two sidecars and no living specs; the diff removes the event-1391 summary absent from the committed timeline and adjusts its derived counts. This does not delete the original workspace event or authorize publication recovery. The full thirty-component check then passes unchanged, as do eighteen component-registry contract tests. Build qualification uses this explicitly derived snapshot, not an assertion that the original workspace or runtime publication is settled. Full distribution, installed Greenfield quality and 60/90/120 proof remain pending. Evidence: /private/tmp/odylith-current-distribution.LLibTP/.

- Current Role Qualification Source Proof (2026-09-10): The existing checker now requires current production-shaped two-role authored observations or one-role clarification, canonical source-bound validation and complete review-payload equality, binary admission, matching provider/profile identities and valid role/shared budgets. Boolean/string consumer durations no longer qualify or count as positives. Initial controls expose seventeen failures; a separate original aggregate characterization exposes six false-positive timing cases. The final adjacent pack passes 208 tests and the root full install unit suite passes 1,327 in 139.11 seconds. Independent frozen-file review finds no bounded P0/P1. Root verifies the unchanged historical DGzOhE receipt remains rejected for unsupported review fields, non-current verdict and mismatched canonical payload; the obsolete call-count objection is removed, not replaced by historical acceptance. Source owners remain below 800 lines and shared test support keeps the adjacent proof-scope file below 1,500. Native observations still cannot prove prompt/schema identity, absolute inter-stage timing or the separate sealed review receipt. Fake providers prove handling, not model quality or fresh 60/90/120 performance. Production prompts, models, deadlines and behavior are unchanged. Evidence: /private/tmp/odylith-profile-checkpoint.ChX3TA/REPORT.md. Existing publication drift still prevents governed projection settlement; release qualification remains open.

- Current Role Qualification Mismatch (2026-09-10): A provider-free invocation of the maintained model-stage observation validator rejects the unchanged retained two-role authored receipt solely because it requires exactly one semantic call. The current production path has initial authoring plus candidate review; source-bound clarification has one call. The release owner summarizes only authoring and ignores review. Correct the existing qualification owner using canonical complete-source/candidate payload binding, current binary admission, exact role/provider/model/effort identity and the available shared-deadline evidence. Do not merely increase the allowed count or import a second semantic interpretation. The historical DGzOhE receipt uses an experimental reviewer and must not become current-protocol proof after this correction. No new provider call, successful repair or fresh 60/90/120 result follows from this characterization. Evidence: /private/tmp/odylith-profile-owner-characterization.aeqZ59/OUTCOME.md. Its old semantic P1 wording is separately superseded by the CB-303 package-level adjudication correction, not by this evaluator change.
