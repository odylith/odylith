...
title: Adaptive Agent Operating Character, Credit-Safe And Benchmark-Proved
...
ordering_rationale: v0.1.11 final alignment: Odylith needs adaptive operating character, continuous learning, benchmark sovereignty, and zero hidden host-model credit burn as one release-bound platform layer.
...
## Problem
Odylith now gives agents more context, tools, delegation, memory, interventions, and benchmark surfaces, but those capabilities are not yet governed by one adaptive runtime model of engineering character under pressure. Guidance behavior is testable, but it is still too easy for discipline, memory, proof, host parity, latency, credit safety, and public accountability to land as adjacent checks instead of one operating loop.
...
- Do not add model/provider calls to hot character checks.
...
- Hot-path character checks report zero provider calls, zero host model calls, zero broad scans, zero full validations, and zero projection expansion.
...
- Codex and Claude share the same character contract across dev, dogfood, and consumer lanes.
...
## Why Now
This is the v0.1.11 brand-level center of gravity: more capability requires more character, and character only matters if it remains adaptive, low-latency, high-signal, credit-safe, and benchmark-proved.
...
- Keep B-096 as the visible intervention umbrella and consume it as one subsystem of the broader character loop.
