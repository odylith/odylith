...
## Proposed Solution
Create a release-bound platform layer that runs local discipline checks, applies immutable hard laws, infers adaptive stance from open-world pressure features, records compact learning, stays silent unless signal is earned, and proves behavior through deterministic validation plus `guidance_behavior` and `discipline` benchmarks.
...
## Success Metrics
- `odylith validate discipline --repo-root .` passes deterministically and credit-free.
- `odylith validate guidance-behavior --repo-root .` remains green as the first pressure-family lane.
- `odylith benchmark --profile quick --family discipline --no-write-report --json` passes before merge.
...
## Interface Changes
- Add `odylith_discipline.v1`.
- Add `odylith_discipline_learning.v1`.
- Add `odylith_discipline_runtime_budget.v1`.
- Add `odylith validate discipline --repo-root . [--case-id ID...] [--json]`.
- Add `odylith discipline status/check/explain`.
- Add `discipline` benchmark family.
