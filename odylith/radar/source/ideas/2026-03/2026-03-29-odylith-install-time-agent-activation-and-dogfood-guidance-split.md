...
status: finished
idea_id: B-016
title: Odylith Install-Time Agent Activation and Dogfood Guidance Split
date: 2026-03-29
priority: P0
commercial_value: 5
product_impact: 5
...
## Opportunity
If install makes Odylith the obvious local operating layer immediately, then the
first consumer session will start with the right guidance, the right skills, and
the right spawn posture already activated. That should improve the practical
product delta versus Codex- or Claude-alone without changing the underlying
agent.
...
- strengthen the managed repo-root `AGENTS.md` install block so Odylith-backed
  workflows and Codex spawn posture are explicit
...
- consumer installs leave behind a repo-root `AGENTS.md` block that explicitly
  defaults grounded work to Odylith guidance, skills, and Codex-native spawn
  when allowed
...
## Open Questions
- should install surface an even more explicit post-install note when Codex
  spawn is active but Claude-native spawn remains unsupported
