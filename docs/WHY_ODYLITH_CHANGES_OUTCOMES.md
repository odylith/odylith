...
| First-time discovery | `odylith show` reads a repo's source structure, import graph, and manifests to suggest component boundaries, workstreams, architecture diagrams, and issues with the exact command to create each one. In empty or thin repos, `odylith greenfield propose` treats the operator request as evidence, compiles typed product intent and the complete first-release governance package, runs deterministic quality gates, and presents one hash-bound `CONFIRM` / `EDIT` / `REJECT` decision. `CONFIRM` commits only those sealed bytes, validates readback, refreshes Radar, Registry, Atlas, and Compass, and directs the operator to the finished project view. |
