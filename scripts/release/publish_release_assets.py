...
def _write_install_script(*, output_path: Path, tag: str, repo: str, odylith_wheel: str) -> None:
    ...
    script_lines = [
        ...
        *PYTHON_ENV_SCRUB_LINES,
        ...
        "  return 1",
        "}",
        "sigstore_stderr_is_continuation() {",
        ...
        "  local asset_path bundle_path stderr_path line folded stripped",
        "  asset_path=\"$1\"",
        "  bundle_path=\"$2\"",
        ...
        "    if [[ -n \"$folded\" ]] && sigstore_stderr_is_continuation \"$folded\" \"$line\" \"$stripped\"; then",
        "      folded=\"$folded $stripped\"",
        "      continue",
        "    fi",
        "    if [[ -n \"$folded\" ]] && ! sigstore_stderr_is_benign \"$folded\"; then",
        "      echo \"$folded\" >&2",
        "    fi",
        "    folded=\"$line\"",
        "  done < \"$stderr_path\"",
