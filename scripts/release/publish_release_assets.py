...
def _write_install_script(*, output_path: Path, tag: str, repo: str, odylith_wheel: str) -> None:
    ...
    script_lines = [
        ...
        *PYTHON_ENV_SCRUB_LINES,
        ...
        "  \"$bootstrap_python\" -m sigstore verify identity \"$tmpdir/release-manifest.json\" --bundle \"$tmpdir/release-manifest.json.sigstore.json\" --offline --cert-identity \"$signer_identity\" --cert-oidc-issuer \"$oidc_issuer\"",
        "  \"$bootstrap_python\" -m sigstore verify identity \"$tmpdir/$runtime_asset_name\" --bundle \"$tmpdir/$runtime_asset_name.sigstore.json\" --offline --cert-identity \"$signer_identity\" --cert-oidc-issuer \"$oidc_issuer\"",
        "  \"$bootstrap_python\" -m sigstore verify identity \"$tmpdir/build-provenance.v1.json\" --bundle \"$tmpdir/build-provenance.v1.json.sigstore.json\" --offline --cert-identity \"$signer_identity\" --cert-oidc-issuer \"$oidc_issuer\"",
        "  \"$bootstrap_python\" -m sigstore verify identity \"$tmpdir/odylith.sbom.spdx.json\" --bundle \"$tmpdir/odylith.sbom.spdx.json.sigstore.json\" --offline --cert-identity \"$signer_identity\" --cert-oidc-issuer \"$oidc_issuer\"",
        "fi",
