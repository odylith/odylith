...
def _write_install_script(*, output_path: Path, tag: str, repo: str, odylith_wheel: str) -> None:
    ...
    script_lines = [
        ...
        *PYTHON_ENV_SCRUB_LINES,
        ...
        'local_release_allow_insecure="${ODYLITH_RELEASE_ALLOW_INSECURE_LOCALHOST:-0}"',
        ...
        "progress_pid=\"\"",
        "progress_supported() {",
        "  [[ \"${ODYLITH_INSTALL_PROGRESS:-1}\" != \"0\" && -t 1 ]]",
        "}",
        "progress_start() {",
        "  local label message",
        "  label=\"$1\"",
        ...
        "        i=$((i + 1))",
        "      done",
        "      printf '\\r  %-6s %s [%s] %ss' \"$label\" \"$message\" \"$bar\" \"$elapsed\"",
        "      sleep 1",
        "      elapsed=$((elapsed + 1))",
        "    done",
        "  ) &",
        ...
        "}",
        "banner() {",
        "  if [[ \"${ODYLITH_INSTALL_BANNER:-1}\" == \"0\" ]]; then",
        "    printf 'Odylith\\n'",
        "    return",
        "  fi",
        "  cat <<'EOF'",
        ...
        "  repo_root_reason=''",
        "  while true; do",
        "    if [[ -f \"$candidate/AGENTS.md\" || -f \"$candidate/CLAUDE.md\" || -f \"$candidate/.claude/CLAUDE.md\" ]]; then",
        "      repo_root=\"$candidate\"",
        "      repo_root_reason='guidance'",
        "      return 0",
        "    fi",
        ...
        "    guidance) ;;",
        "    git)",
        "      say \"setup  Adding Odylith guidance to this Git repo.\"",
        "      ;;",
        "    folder)",
        "      say \"setup  Adding Odylith guidance to this folder.\"",
        "      say \"note   Git-aware help turns on after this folder has a .git directory.\"",
        "      ;;",
        "  esac",
        "}",
        ...
        "proxy_env_summary() {",
        "  local found=0 name",
        "  for name in HTTPS_PROXY https_proxy HTTP_PROXY http_proxy NO_PROXY no_proxy SSL_CERT_FILE CURL_CA_BUNDLE REQUESTS_CA_BUNDLE; do",
        "    if [[ -n \"${!name:-}\" ]]; then",
        "      if [[ \"$found\" -eq 0 ]]; then",
        "        printf '%s' \"$name\"",
        "      else",
        ...
        "}",
        "emit_fetch_failure() {",
        "  local url stderr_path env_summary",
        "  url=\"$1\"",
        "  stderr_path=\"$2\"",
        "  echo \"Odylith could not download a release asset.\" >&2",
        "  echo \"URL: $url\" >&2",
        "  if [[ -s \"$stderr_path\" ]]; then",
        "    sed -n '1,8p' \"$stderr_path\" >&2",
        "  fi",
        "  env_summary=\"$(proxy_env_summary || true)\"",
        "  if [[ -n \"$env_summary\" ]]; then",
        "    echo \"Detected proxy/TLS environment: $env_summary\" >&2",
        "  fi",
        "  echo \"Check VPN, proxy, firewall, TLS inspection, and certificate settings. curl honors HTTPS_PROXY, HTTP_PROXY, NO_PROXY, SSL_CERT_FILE, and CURL_CA_BUNDLE.\" >&2",
        "  if allow_local_http_asset \"$url\"; then",
        "    echo \"For local release testing, keep the local HTTP server running and set ODYLITH_RELEASE_ALLOW_INSECURE_LOCALHOST=1.\" >&2",
        "  fi",
        "}",
        "fetch_asset() {",
        "  local url destination stderr_path scheme",
        "  url=\"$1\"",
        "  destination=\"$2\"",
        "  mkdir -p \"$(dirname \"$destination\")\"",
        "  stderr_path=\"$tmpdir/curl-$(basename \"$destination\").stderr\"",
        "  rm -f \"$stderr_path\"",
        "  scheme=\"$(release_url_scheme \"$url\")\"",
        "  if allow_local_http_asset \"$url\"; then",
        "    if [[ \"$local_release_allow_insecure\" != \"1\" ]]; then",
        "      echo \"Odylith refused insecure localhost release assets. Set ODYLITH_RELEASE_ALLOW_INSECURE_LOCALHOST=1 only for local maintainer testing.\" >&2",
        "      return 2",
        "    fi",
        "    if ! curl --fail --show-error --silent --location --retry 3 --retry-connrefused --retry-delay 1 \"$url\" -o \"$destination\" 2>\"$stderr_path\"; then",
        "      rm -f \"$destination\"",
        "      emit_fetch_failure \"$url\" \"$stderr_path\"",
        "      return 2",
        "    fi",
        "    return",
        "  fi",
        "  if [[ \"$scheme\" != \"https\" ]]; then",
        "    echo \"Odylith refused non-HTTPS release assets: $url\" >&2",
        "    echo \"Use HTTPS release hosting, or localhost HTTP with ODYLITH_RELEASE_ALLOW_INSECURE_LOCALHOST=1 for maintainer-only local testing.\" >&2",
        "    return 2",
        "  fi",
        "  if ! curl --fail --show-error --silent --location --proto '=https' --tlsv1.2 --retry 3 --retry-connrefused --retry-delay 1 \"$url\" -o \"$destination\" 2>\"$stderr_path\"; then",
        "    rm -f \"$destination\"",
        "    emit_fetch_failure \"$url\" \"$stderr_path\"",
        "    return 2",
        "  fi",
        ...
        "  value=\"${value//$'\\r'/}\"",
        "  while [[ \"$value\" =~ $ansi_re ]]; do",
        "    value=\"${value/${BASH_REMATCH[0]}/}\"",
        "  done",
        "  printf '%s' \"$value\"",
        "}",
        "sigstore_log_is_benign() {",
        "  local line",
        "  line=\"$(sigstore_normalize_line \"$1\")\"",
        (
            "  if printf '%s\\n' \"$line\" | grep -Eiq "
            "'(WARNING[[:space:]]+)?(Failed to load a trusted root key:[[:space:]]*)?"
            "unsupported([[:space:]]+[^[:space:]]+:[0-9]+)?[[:space:]]+key type:[[:space:]]*7'; then"
        ),
        "    return 0",
        "  fi",
        ...
        "  return 1",
        "}",
        "sigstore_log_is_continuation() {",
        "  local previous line stripped",
        "  previous=\"$1\"",
        "  line=\"$2\"",
        "  stripped=\"$3\"",
        ...
        "    return 0",
        "  fi",
        "  if [[ \"$previous\" == \"WARNING\" || \"$previous\" == \"WARNING:\" ]]; then",
        "    [[ \"$stripped\" == Failed\\ to\\ load\\ a\\ trusted\\ root\\ key* ]] && return 0",
        "  fi",
        "  if [[ \"$previous\" == *\"Failed to load a trusted root key\"* ]]; then",
        "    case \"$stripped\" in",
        "      key\\ type:*|unsupported\\ *|trust.py:*) return 0 ;;",
        "    esac",
        "  fi",
        "  return 1",
        "}",
        "emit_sigstore_log() {",
        "  local log_path stream line folded stripped",
        "  log_path=\"$1\"",
        "  stream=\"$2\"",
        "  folded=\"\"",
        "  while IFS= read -r line || [[ -n \"$line\" ]]; do",
        "    line=\"$(sigstore_normalize_line \"$line\")\"",
        "    [[ -z \"$line\" ]] && continue",
        "    stripped=\"${line#\"${line%%[![:space:]]*}\"}\"",
        "    if [[ -n \"$folded\" ]] && sigstore_log_is_continuation \"$folded\" \"$line\" \"$stripped\"; then",
        "      folded=\"$folded $stripped\"",
        "      continue",
        "    fi",
        "    if [[ -n \"$folded\" ]] && ! sigstore_log_is_benign \"$folded\"; then",
        "      if [[ \"$stream\" == \"stdout\" ]]; then echo \"$folded\"; else echo \"$folded\" >&2; fi",
        "    fi",
        "    folded=\"$line\"",
        "  done < \"$log_path\"",
        "  if [[ -n \"$folded\" ]] && ! sigstore_log_is_benign \"$folded\"; then",
        ...
        "  local asset_path bundle_path stdout_path stderr_path",
        "  asset_path=\"$1\"",
        "  bundle_path=\"$2\"",
        "  stdout_path=\"$tmpdir/sigstore-stdout.log\"",
        "  stderr_path=\"$tmpdir/sigstore-stderr.log\"",
        "  if ! \"$bootstrap_python\" -m sigstore verify identity \"$asset_path\" --bundle \"$bundle_path\" --offline --cert-identity \"$signer_identity\" --cert-oidc-issuer \"$oidc_issuer\" >\"$stdout_path\" 2>\"$stderr_path\"; then",
        "    emit_sigstore_log \"$stdout_path\" stderr",
        "    emit_sigstore_log \"$stderr_path\" stderr",
        "    rm -f \"$stdout_path\" \"$stderr_path\"",
        "    exit 2",
        "  fi",
        "  emit_sigstore_log \"$stdout_path\" stdout",
        "  emit_sigstore_log \"$stderr_path\" stderr",
        "  rm -f \"$stdout_path\" \"$stderr_path\"",
        "}",
        ...
        "fi",
        'say "Preparing this repo."',
        "describe_repo_root_choice",
        'say "host   $platform_name."',
        'say "safe   Your repo'"'"'s own toolchain stays untouched."',
        'if [[ "$release_version" == "latest" ]]; then',
        '  say "version latest available."',
        "else",
        '  say "version v$release_version."',
        "fi",
        'if [[ -n "$local_release_base_url" ]]; then',
        '  say "local  $local_release_base_url"',
        "fi",
        'if [[ -x "$repo_root/.odylith/bin/odylith" ]]; then',
        '  say "refresh Existing Odylith install found."',
        "fi",
        ...
        'progress_start "fetch" "Downloading Odylith."',
        ...
        "  exit 2",
        "fi",
        'progress_done "fetch" "Download complete."',
        'progress_start "check" "Checking the release."',
        ...
        *validate_release_script,
        ...
        'progress_done "check" "Release verified."',
        ...
        'progress_start "setup" "Installing local runtime."',
        ...
        'progress_done "setup" "Local runtime ready."',
        'say "setup  Writing repo files and launchers."',
        ...
        "cat > \"$tmpdir/read_install_versions.py\" <<'PY'",
        "import json",
        "import sys",
        "from pathlib import Path",
        "",
        "_repo_root, pin_path, install_state_path = sys.argv[1:4]",
        "",
        "def _read_json(path_token: str) -> dict[str, object]:",
        ...
        "state = _read_json(install_state_path)",
        "print('\\t'.join([",
        "    str(pin.get('odylith_version') or '').strip(),",
        "    str(state.get('active_version') or '').strip(),",
        "    str(state.get('last_known_good_version') or '').strip(),",
        "]))",
        "PY",
        'installed_pin_version=""',
        'installed_active_version=""',
        'installed_last_known_good_version=""',
        'if [[ -f "$pin_path" && -f "$install_state_path" ]]; then',
        '  installed_versions="$("$version_root/bin/python" "$tmpdir/read_install_versions.py" "$repo_root" "$pin_path" "$install_state_path" || true)"',
        '  IFS=$'"'"'\\t'"'"' read -r installed_pin_version installed_active_version installed_last_known_good_version <<< "$installed_versions"',
        "fi",
        'if [[ -f "$pin_path" && -f "$install_state_path" && -f "$customer_tree_path" ]]; then',
        '  if [[ "$installed_pin_version" == "$release_version" && "$installed_active_version" == "$release_version" ]]; then',
        '    ODYLITH_INSTALL_COMPACT=1 ODYLITH_BOOTSTRAP_RUNTIME_PRESTAGED=1 "$version_root/bin/python" -m odylith.cli install --repo-root "$repo_root" --version "$release_version"',
        "  else",
        ...
        '    say "resume Completing local install."',
        "  fi",
        '  rm -f "$install_state_path" "$state_root/runtime/current"',
        '  if [[ -L "$migration_state_dir" ]]; then',
        '    rm -f "$migration_state_dir"',
        '  elif [[ -d "$migration_state_dir" && ! -L "$repo_root/.odylith" && ! -L "$repo_root/.odylith/state" ]]; then',
        '    rm -rf "$migration_state_dir"',
        "  fi",
        '  ODYLITH_INSTALL_COMPACT=1 ODYLITH_BOOTSTRAP_RUNTIME_PRESTAGED=1 "$version_root/bin/python" -m odylith.cli install --repo-root "$repo_root" --version "$release_version"',
        "fi",
