...
- Hook context is not chat-visible proof. Before claiming intervention UX, run or cite `odylith codex intervention-status` or `odylith claude intervention-status`; end-to-end proof requires `Activation: ready` plus chat visibility. When uncertain, run or cite `odylith codex visible-intervention` or `odylith claude visible-intervention` and show that Markdown directly. Existing sessions may not hot-reload hooks or guidance.
