...
- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt "<request>"` for no-write Product Intent Confirmation. In chat, write live product story, actors, systems, assumptions, ambiguities, and a clear Next step block: Confirm to apply, Edit to correct, Reject to stop. Before confirmation, no backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or prompt-only `greenfield create`. After confirmation, run `odylith greenfield propose --confirm-intent --format json` internally as the proposal contract; do not show that contract as a second approval step, and do not inspect Odylith source files, Python modules, local examples, `.odylith`, or bundle assets to discover schemas. Author an internal proposal payload from the confirmed product shape, then run `odylith greenfield apply --repo-root . --proposal-file .odylith/runtime/greenfield/active-proposal.v1.json --confirm`; apply validates, Tribunal-gates, writes records, and refreshes surfaces from the same confirmation. Do not ask the operator to inspect proposal JSON or confirm a second time unless they explicitly request a review artifact. No canned domain families, scaffolds, templates, or code. Surface only the human-readable created-record summary or validation/Tribunal blockers.
