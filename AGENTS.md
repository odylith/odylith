...
- Empty/thin prompts use `odylith greenfield propose --repo-root . --prompt "<request>"` for project-first Product Intent Confirmation as sectioned Markdown with `## Choose one command`. Include `Start your reply with exactly one command: **CONFIRM**, **EDIT**, or **REJECT**.`, **Command: `CONFIRM`**, **Command: `EDIT`**, **Command: `REJECT`**, and `Reply starts with: CONFIRM`, `Reply starts with: EDIT`, `Reply starts with: REJECT`. Pre-confirm: staging only. Save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`; run `odylith greenfield compile-transaction --repo-root . --prompt "<request>" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --output .odylith/runtime/greenfield/product-create-transaction.v1.json --release 0.0.1`; it may write `.odylith/runtime/greenfield/confirmed-intent.json` and seal the ProductCreateTransaction before governed writes. CONFIRM runs `odylith greenfield create --repo-root . --transaction-file .odylith/runtime/greenfield/product-create-transaction.v1.json --transaction-hash <hash> --confirm`; create verifies the compiler receipt, hash, and repo preconditions, writes only sealed bytes under rollback guard, and validates readback. **EDIT** adds evidence and rebuilds; **REJECT** stops with no writes. Do not inspect Odylith source, hand-author/repair proposal JSON, narrate parser/schema retries, or stop after repairable compile issues. Do not ask operator to inspect proposal JSON or add a second confirmation. Surface only final summary or blockers.
...
- Maintainer mode: pinned dogfood is default proof; detached `source-local` is explicit dev.
