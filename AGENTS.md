...
- Empty/thin prompts use `odylith greenfield propose --repo-root . --prompt "<request>"` for project-first Product Intent Confirmation as sectioned Markdown: Product story, State object, First complete path, Proof boundary, and `## Choose one command`. Include `Start your reply with one clear command: **CONFIRM**, **EDIT**, or **REJECT**.`, with sections: **Command: `CONFIRM`**, **Command: `EDIT`**, and **Command: `REJECT`**. Before confirmation: no governed writes; staging only. Save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`; run `odylith greenfield compile-transaction --repo-root . --prompt "<request>" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --output .odylith/runtime/greenfield/product-create-transaction.v1.json --release 0.0.1`; it may write `.odylith/runtime/greenfield/confirmed-intent.json`, then hashes gated ProductCreateTransaction. On **CONFIRM**, run `odylith greenfield create --repo-root . --transaction-file .odylith/runtime/greenfield/product-create-transaction.v1.json --transaction-hash <hash> --confirm`; create verifies hash, writes atomically, validates readback, and reports. **EDIT** means corrections after command are evidence; rebuild. **REJECT** means stop with no governed records. Do not inspect Odylith source, hand-author/repair proposal JSON, narrate parser/schema retries, or stop after repairable transaction-compile issues. Do not ask operator to inspect proposal JSON or add a second confirmation. Surface only final summary or blockers.
