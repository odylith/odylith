...
- Governance-learning is mandatory across maintainer and consumer lanes, Codex and Claude, and shipped guidance. Durable errors, failed mechanisms, bad artifacts, semantic drift, gate failures, decisions, validation results, and release risks must update the appropriate Casebook, Radar, Registry, Atlas, or Compass record before continuing, committing, releasing, or claiming completion. Before a fix, search existing truth, read prior failed mechanisms, and do not repeat a fix path that governance already shows failed.
