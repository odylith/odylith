...
- Adaptive Agent Operating Character is the v0.1.11 shared Codex/Claude behavior contract: hard laws are deterministic, runtime pressure is open-world, stance is local and credit-safe, passing checks stay quiet, and durable learning requires validator, benchmark, or Tribunal/governance proof. Use `odylith character status/check/explain`, `odylith validate agent-operating-character --repo-root .`, and `odylith benchmark --profile quick --family agent_operating_character --no-write-report --json`; none of those character hot paths may call host models, providers, subagents, broad scans, full validation, or projection expansion.
