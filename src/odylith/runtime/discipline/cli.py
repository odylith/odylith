...
from odylith.runtime.discipline.support import SUPPORTED_HOST_FAMILIES, SUPPORTED_LANES, host_lane_matrix


DECISION_CACHE_RELATIVE = Path(".odylith/cache/discipline/decisions")
...
def run_discipline(argv: Sequence[str] | None = None) -> int:
    ...
    if args.discipline_command == "status":
        summary = discipline_runtime.runtime_summary(repo_root=repo_root)
    ...
    if args.discipline_command == "check":
        ...
        if not intent_path.is_file():
            ...
            return 1
        payload = evaluate_discipline_move(
