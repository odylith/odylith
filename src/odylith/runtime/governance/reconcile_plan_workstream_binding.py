...
def _active_plan_index_layout(
    *, repo_root: Path, plan_index_path: Path,
) -> tuple[list[str], Mapping[str, Any], int, int]:
    """Validate the complete source table before registration or shared no-op admission."""
    ...
    active_start, active_end = _find_section_bounds(lines, "## Active Plans")
    indexes = [idx for idx in range(active_start + 1, active_end) if lines[idx].strip().startswith("|")]
    if (len(indexes) < 2 or indexes != list(range(indexes[0], indexes[-1] + 1))
            or len(indexes) != len(section.get("rows", ())) + 2):
        raise ValueError(f"{plan_index_path}: registration requires one contiguous Active Plans table")
    cells = [_split_table_cells(lines[idx].strip()) for idx in indexes]
    if any(len(row) != len(backlog_contract._PLAN_COLS) or not lines[idx].strip().endswith("|")
           for idx, row in zip(indexes, cells, strict=True)):
        raise ValueError(f"{plan_index_path}: malformed Active Plans row")
    if any(len(cell.strip(":")) < 3 or set(cell.strip(":")) != {"-"} for cell in cells[1]):
        raise ValueError(f"{plan_index_path}: malformed Active Plans table separator")
    return lines, section, indexes[0], indexes[-1] + 1
...
def _prepare_active_plan_registration(
    *, repo_root: Path, plan_index_path: Path, plan_paths: Sequence[str],
    ...
    if not rows:
        return [], None
    lines, section, table_start, table_end = _active_plan_index_layout(repo_root=repo_root, plan_index_path=plan_index_path)
...
def reconcile_plan_workstream_binding(
    *,
    ...
    rows_by_plan: dict[str, list[dict[str, str]]] = {}
    for row in active_rows:
        rows_by_plan.setdefault(str(row.get("Plan", "")).strip(), []).append(row)
    ...
    for row in registration_rows:
        rows_by_plan[row["Plan"]] = [row]
    candidate_rows = [*active_rows, *registration_rows]
    no_op_paths: set[str] = set()
    for plan_path in touched_plan_paths:
        rows = rows_by_plan.get(plan_path, [])
        if not rows:
            continue
        all_noops = True
        for row in rows:
            ...
            if (spec is None or spec.status.lower() not in {"queued", "planning", "implementation"}
                    or _active_plan_block_reason(repo_root=repo_root, plan_path=plan_path)):
                all_noops = False
                continue
            if sum(str(item.get("Backlog", "")).strip().strip("`") == spec.idea_id for item in candidate_rows) != 1:
            ...
            _lines, tables = _execution_repair_layout(backlog_index_path, repo_root=repo_root, idea_id=spec.idea_id)
            matches_execution = spec.status.lower() in {"planning", "implementation"} and any(
                cells[1] == spec.idea_id and cells[_BACKLOG_STATUS_COL_INDEX] == spec.status.lower()
                for _idx, cells in tables["execution"][1]
            )
            all_noops = all_noops and matches_execution
        # Historical shared bindings are safe only when every member needs no
        # mutation. Never choose one member as the group's write authority.
        if len(rows) != 1 and not all_noops:
            raise ValueError(f"{plan_index_path}: execution repair requires a unique indexed active-plan binding")
        if len(rows) > 1:
            _active_plan_index_layout(repo_root=repo_root, plan_index_path=plan_index_path)
        if all_noops:
            no_op_paths.add(plan_path)
