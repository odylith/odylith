"""Typed finding collectors for greenfield pre-confirm review gates."""
...
def completion_review_findings(
    ...
    *,
    ...
    """Collect typed findings from proposal-level pre-confirm validators."""

    findings: list[GreenfieldReviewFinding] = []
    findings.extend(_preconfirm_contract_findings(proposal, rendered_specs=rendered_specs))
    ...
    *,
    ...
    if int(proposal.get("provider_calls") or 0) != 0:
        return review_findings_from_messages(
            ["pre-confirm completion must be provider-free by default"],
