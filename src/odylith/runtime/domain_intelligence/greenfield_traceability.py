...
def _patch_sections(
    *,
    ...
    validation_items = (
        _validation_section_items(row.get("validation", []))
        or _validation_section_items(row.get("validation_gate", []))
        or _validation_section_items(row.get("success_metrics", []))
...
def _validation_section_items(value: Any) -> list[str]:
    return [
        normalized
        for item in _section_items(value)
        if (normalized := normalize_cover_article_language(item))
    ]
...
def _first_implementation_step_lines(first_slice: str) -> list[str]:
    ...
    if not text:
        return []
    if len(text) <= 180 and text.count(",") < 3:
...
def _bounded_scope_lines(value: str) -> list[str]:
    ...
    if completion.casefold() not in scope.casefold():
        rows.append(f"Completion proof includes {completion}")
    return rows
