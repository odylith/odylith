...
def _entity_from_row(*, kind: str, row: Mapping[str, Any]) -> dict[str, Any]:
    if kind == "workstream":
        ...
        return {
            ...
            "metadata": metadata if isinstance(metadata, context_engine_store.Mapping) else {},
        }
    if kind == "plan":
        return {
            "kind": "plan",
            "entity_id": str(row["plan_path"]),
            "title": context_engine_store.Path(str(row["plan_path"])).name,
            "status": str(row["status"]),
            "path": str(row["source_path"]),
            "section": str(row["section"]),
    ...
    if kind == "release":
        ...
        return {
            ...
            "aliases": context_engine_store._json_list(str(row.get("aliases_json", ""))),
            "active_workstreams": context_engine_store._json_list(str(row.get("active_workstreams_json", ""))),
            "metadata": metadata if isinstance(metadata, context_engine_store.Mapping) else {},
        }
    if kind == "bug":
        ...
        return {
            ...
            "components": context_engine_store._parse_component_tokens(str(row["components"])),
        }
    if kind == "diagram":
        metadata = context_engine_store.json.loads(str(row["metadata_json"] or "{}"))
        return {
    ...
    if kind == "component":
        ...
        return {
            ...
            "owner": str(row["owner"]),
            "aliases": context_engine_store._json_list(str(row["aliases_json"])),
            "workstreams": context_engine_store._json_list(str(row["workstreams_json"])),
            "diagrams": context_engine_store._json_list(str(row["diagrams_json"])),
            "metadata": metadata if isinstance(metadata, context_engine_store.Mapping) else {},
        }
    if kind in context_engine_store._ENGINEERING_NOTE_KIND_SET:
        metadata = context_engine_store.json.loads(str(row["metadata_json"] or "{}"))
        return {
            "kind": kind,
            ...
            "workstreams": context_engine_store._json_list(str(row["workstreams_json"])),
            "path_refs": context_engine_store._json_list(str(row["path_refs_json"])),
            "metadata": metadata if isinstance(metadata, context_engine_store.Mapping) else {},
        }
    if kind == "test":
        metadata = context_engine_store.json.loads(str(row["metadata_json"] or "{}"))
        return {
            ...
            "target_paths": context_engine_store._json_list(str(row["target_paths_json"])),
            "metadata": metadata if isinstance(metadata, context_engine_store.Mapping) else {},
        }
...
def _entity_by_kind_id(
    ...
    *,
    ...
    normalized_kind = context_engine_store._normalize_entity_kind(kind)
    ...
    if normalized_kind == "workstream":
        ...
        if selector_key:
            ...
            for candidate in connection.execute("SELECT * FROM releases").fetchall():
                aliases = {item.casefold() for item in context_engine_store._json_list(str(candidate.get("aliases_json", "")))}
                candidate_keys = {
    ...
    elif normalized_kind in context_engine_store._ENGINEERING_NOTE_KIND_SET:
...
def _existing_repo_local_file(
    *,
    ...
    normalized_path = context_engine_store._normalize_repo_token(path_ref, repo_root=repo_root)
    if not normalized_path:
        return ""
    normalized_candidate = context_engine_store.Path(normalized_path)
...
def _synthesized_local_path_entity(
    *,
    ...
    return {
        ...
        "title": context_engine_store.Path(normalized_path).name,
        "status": "",
...
def _entity_kind_matches(entity: Mapping[str, Any] | None, *, expected_kind: str) -> bool:
    if not isinstance(entity, context_engine_store.Mapping):
        return False
    ...
    *,
    ...
    for candidate in connection.execute("SELECT * FROM workstreams").fetchall():
        if normalized_path in {
            context_engine_store._normalize_repo_token(str(candidate["source_path"]), repo_root=repo_root),
            context_engine_store._normalize_repo_token(str(candidate["idea_file"]), repo_root=repo_root),
        }:
    ...
    for entity_kind, query in (
        ...
        for candidate in connection.execute(query).fetchall():
            candidate_paths = set()
            if entity_kind == "plan":
                candidate_paths.add(context_engine_store._normalize_repo_token(str(candidate["plan_path"]), repo_root=repo_root))
                candidate_paths.add(context_engine_store._normalize_repo_token(str(candidate["source_path"]), repo_root=repo_root))
            elif entity_kind == "bug":
                candidate_paths.add(context_engine_store._normalize_repo_token(str(candidate["link_target"]), repo_root=repo_root))
                candidate_paths.add(context_engine_store._normalize_repo_token(str(candidate["source_path"]), repo_root=repo_root))
            elif entity_kind == "diagram":
                candidate_paths.update(
                    {
                        context_engine_store._normalize_repo_token(str(candidate["source_mmd"]), repo_root=repo_root),
                        context_engine_store._normalize_repo_token(str(candidate["source_svg"]), repo_root=repo_root),
                        context_engine_store._normalize_repo_token(str(candidate["source_png"]), repo_root=repo_root),
                    }
                )
            elif entity_kind == "component":
                candidate_paths.add(context_engine_store._normalize_repo_token(str(candidate["spec_ref"]), repo_root=repo_root))
            elif entity_kind == "test":
                candidate_paths.add(context_engine_store._normalize_repo_token(str(candidate["test_path"]), repo_root=repo_root))
...
def _unique_entity_by_path_alias(
    ...
    *,
    ...
    if not raw_ref or "/" in raw_ref:
        return None, ""
    alias = context_engine_store.Path(raw_ref).name
    ...
    def _path_matches(path_ref: str) -> bool:
        normalized_path = context_engine_store._normalize_repo_token(path_ref, repo_root=repo_root)
        return bool(normalized_path) and context_engine_store.Path(normalized_path).name.casefold() == alias_casefold

    def _add(entity: Mapping[str, Any] | None) -> None:
        if not isinstance(entity, context_engine_store.Mapping):
            return
        row = context_engine_store._search_row_from_entity(entity)
...
def _projection_exact_search_results(
    ...
    *,
    ...
    if not raw_query:
        return []
    normalized_query = context_engine_store._normalize_repo_token(raw_query, repo_root=repo_root)
    lowered = raw_query.casefold()
    compact_lookup = context_engine_store._context_lookup_key(raw_query)
    ...
    candidate_kinds = tuple(allowed) if allowed else (
        ...
        "diagram",
        "component",
        *context_engine_store._ENGINEERING_NOTE_KINDS,
        "test",
    )
...
def _repo_scan_candidate_search_results(
    ...
    *,
    ...
    query_tokens = {token.casefold() for token in context_engine_store.re.findall(r"[A-Za-z0-9_./:-]+", str(query or "")) if token}
    results: list[dict[str, Any]] = []
    seen: set[tuple[str, str, str]] = set()
    for rank, hit in enumerate(fallback_scan.get("results", []), start=1):
        ...
        if entity is None:
            if bool(context_engine_store._path_signal_profile(path_ref).get("shared")):
                continue
            inferred_kind = context_engine_store._repo_scan_inferred_kind(path_ref)
        ...
        row = context_engine_store._search_row_from_entity(
            entity,
...
def _resolve_context_entity(
    ...
    *,
    ...
    normalized_ref = context_engine_store._normalize_repo_token(raw_ref, repo_root=repo_root)
    ...
    if normalized_kind in {
        ...
        "diagram",
        "component",
        *context_engine_store._ENGINEERING_NOTE_KIND_SET,
        "test",
    }:
    ...
    if context_engine_store._WORKSTREAM_ID_RE.fullmatch(raw_ref.upper()):
        entity = _fetch_by_id("workstream", raw_ref.upper())
        if entity is not None:
            return entity, [], {"resolution_mode": "workstream_id"}
    if context_engine_store._DIAGRAM_ID_RE.fullmatch(raw_ref.upper()):
    ...
    compact_lookup = context_engine_store._context_lookup_key(raw_ref)
    for row in connection.execute("SELECT * FROM releases").fetchall():
        aliases = context_engine_store._release_lookup_aliases(dict(row), repo_root=repo_root)
        if lowered in aliases or (compact_lookup and compact_lookup in aliases):
    ...
    search_payload = context_engine_store.search_entities_payload(repo_root=repo_root, query=raw_ref, limit=5, runtime_mode="auto")
    matches = [dict(row) for row in search_payload.get("results", []) if isinstance(row, context_engine_store.Mapping)]
    ...
    return None, matches, {
        ...
        if isinstance(search_payload.get("fallback_scan"), context_engine_store.Mapping)
        else {},
    }
    ...
    *,
    ...
    def _add_kind_id(kind: str, entity_id: str) -> None:
        ...
        if resolved is not None:
            _add(context_engine_store._summarize_entity(resolved))
            return
            ...
            _add(
                {
                    "kind": kind,
                    "entity_id": entity_id,
                    "title": context_engine_store.Path(entity_id).name,
                    "path": entity_id,
                    "status": "",
                }
            )
    ...
    if kind == "workstream":
        ...
        for row in connection.execute("SELECT * FROM components").fetchall():
            if workstream_id in set(context_engine_store._json_list(str(row["workstreams_json"]))):
        ...
        for row in connection.execute("SELECT * FROM components").fetchall():
            if diagram_id in set(context_engine_store._json_list(str(row["diagrams_json"]))):
                _add(context_engine_store._summarize_entity(_entity_from_row(kind="component", row=row)))
    elif kind == "plan":
        backlog = str(entity.get("backlog", "")).strip().strip("`")
        if context_engine_store._WORKSTREAM_ID_RE.fullmatch(backlog):
...
def _recent_context_events(
    ...
    *,
    ...
    results: list[dict[str, Any]] = []
    normalized_paths = {context_engine_store.Path(item).as_posix() for item in artifact_paths if item}
    for row in rows:
        event_workstreams = set(context_engine_store._json_list(str(row["workstreams_json"])))
        event_components = set(context_engine_store._json_list(str(row["components_json"])))
        event_artifacts = {context_engine_store.Path(item).as_posix() for item in context_engine_store._json_list(str(row["artifacts_json"]))}
        if not (
...
def _delivery_context_rows(
    ...
    *,
    ...
    for scope_type, scope_id in sorted(candidates):
        ...
        if payload is None:
            continue
        decoded = context_engine_store.json.loads(str(payload["payload_json"]))
        if isinstance(decoded, context_engine_store.Mapping):
...
def load_context_dossier(
    *,
    ...
    started_at = context_engine_store.time.perf_counter()
    runtime_ready = _warm_runtime(repo_root=root, runtime_mode=runtime_mode, reason="context", scope="reasoning")
    if not runtime_ready:
        fallback_scan = context_engine_store._full_scan_guidance(
            repo_root=root,
        ...
        return {
            "query": str(ref or "").strip(),
            "requested_kind": context_engine_store._normalize_entity_kind(kind),
            "resolved": False,
            "matches": [],
            "lookup": {
    ...
    try:
        ...
        if odylith_ablation_active:
            matches = [
                dict(row)
                for row in matches
                if isinstance(row, context_engine_store.Mapping) and not context_engine_store._odylith_runtime_entity_suppressed(repo_root=root, entity=row)
            ]
            if isinstance(entity, context_engine_store.Mapping) and context_engine_store._odylith_runtime_entity_suppressed(repo_root=root, entity=entity):
                entity = None
                lookup = {
                    **dict(lookup),
            ...
            if entity is None and context_engine_store._odylith_query_targets_disabled(repo_root=root, query=str(ref or "").strip()):
                return {
                    "query": str(ref or "").strip(),
                    "requested_kind": context_engine_store._normalize_entity_kind(kind),
                    "resolved": False,
                    ...
                    "fallback_scan": {
                        "performed": False,
                        "terms": context_engine_store._full_scan_terms(repo_root=root, query=str(ref or "").strip()),
        ...
        if entity is None:
            ...
            fallback_scan = (
                ...
                if isinstance(lookup.get("fallback_scan"), context_engine_store.Mapping)
                else context_engine_store._full_scan_guidance(
                    repo_root=root,
        ...
        record_runtime_timing(
            ...
            category="reasoning",
            operation="context",
            duration_ms=(context_engine_store.time.perf_counter() - started_at) * 1000.0,
            metadata={
                "query": str(ref or "").strip(),
                "requested_kind": context_engine_store._normalize_entity_kind(kind),
            },
