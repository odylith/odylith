        ...
        compact.append(
            {
                ...
                for key, value in {
                    ...
                    "shared_paths": context_engine_store._normalized_string_list(row.get("shared_paths"))[:3],
                    "shared_surfaces": context_engine_store._normalized_string_list(row.get("shared_surfaces"))[:3],
                }.items()
...
def _hot_path_keep_architecture_audit(audit: Mapping[str, Any]) -> bool:
    if not isinstance(audit, context_engine_store.Mapping) or not audit:
        return False
...
def _hot_path_workstream_selection(payload: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(payload, context_engine_store.Mapping):
        return {}
    selection = dict(payload.get("workstream_selection", {})) if isinstance(payload.get("workstream_selection"), context_engine_store.Mapping) else {}
    context_packet = (
        dict(payload.get("context_packet", {}))
        if isinstance(payload.get("context_packet"), context_engine_store.Mapping)
        else {}
    )
    compact_state, compact_workstream = context_engine_store._compact_selection_state_parts(
    ...
    inferred_workstream = context_engine_store._payload_workstream_hint(payload, include_selection=False)
    ...
    selected_workstream = (
        ...
        if isinstance(selection.get("selected_workstream"), context_engine_store.Mapping)
        else {}
    )
    top_candidate = (
        dict(selection.get("top_candidate", {}))
        if isinstance(selection.get("top_candidate"), context_engine_store.Mapping)
        else {}
    )
    ...
    return {
        ...
        "competing_candidates": [
            ...
            if isinstance(selection.get("competing_candidates"), list) and isinstance(row, context_engine_store.Mapping)
        ][:3],
    }
...
def _compact_hot_path_workstream_selection(selection: Mapping[str, Any]) -> dict[str, Any]:
    ...
    selected = dict(selection.get("selected_workstream", {})) if isinstance(selection.get("selected_workstream"), context_engine_store.Mapping) else {}
    top_candidate = dict(selection.get("top_candidate", {})) if isinstance(selection.get("top_candidate"), context_engine_store.Mapping) else {}
...
def _compact_hot_path_retrieval_plan(plan: Mapping[str, Any]) -> dict[str, Any]:
    ...
    selected_domains = context_engine_store._normalized_string_list(plan.get("selected_domains"))
    if selected_domains:
        compact["selected_domains"] = selected_domains[:4]
    miss_recovery = dict(plan.get("miss_recovery", {})) if isinstance(plan.get("miss_recovery"), context_engine_store.Mapping) else {}
    if miss_recovery:
...
def _compact_hot_path_packet_quality(summary: Mapping[str, Any]) -> dict[str, Any]:
    utility_profile = dict(summary.get("utility_profile", {})) if isinstance(summary.get("utility_profile"), context_engine_store.Mapping) else {}
    token_efficiency = dict(utility_profile.get("token_efficiency", {})) if isinstance(utility_profile.get("token_efficiency"), context_engine_store.Mapping) else {}
    intent_profile = dict(summary.get("intent_profile", {})) if isinstance(summary.get("intent_profile"), context_engine_store.Mapping) else {}
    context_density = dict(summary.get("context_density", {})) if isinstance(summary.get("context_density"), context_engine_store.Mapping) else {}
    reasoning_readiness = (
        dict(summary.get("reasoning_readiness", {})) if isinstance(summary.get("reasoning_readiness"), context_engine_store.Mapping) else {}
    )
    evidence_quality = dict(summary.get("evidence_quality", {})) if isinstance(summary.get("evidence_quality"), context_engine_store.Mapping) else {}
    actionability = dict(summary.get("actionability", {})) if isinstance(summary.get("actionability"), context_engine_store.Mapping) else {}
    validation_pressure = (
        dict(summary.get("validation_pressure", {})) if isinstance(summary.get("validation_pressure"), context_engine_store.Mapping) else {}
    )
    evidence_diversity = (
        dict(summary.get("evidence_diversity", {})) if isinstance(summary.get("evidence_diversity"), context_engine_store.Mapping) else {}
    )
    compact = {
        key: value
        for key, value in {
...
def _compact_hot_path_intent(summary: Mapping[str, Any]) -> dict[str, Any]:
    intent = dict(summary.get("intent", {})) if isinstance(summary.get("intent"), context_engine_store.Mapping) else {}
    packet_quality = dict(summary.get("packet_quality", {})) if isinstance(summary.get("packet_quality"), context_engine_store.Mapping) else {}
    intent_profile = dict(packet_quality.get("intent_profile", {})) if isinstance(packet_quality.get("intent_profile"), context_engine_store.Mapping) else {}
    compact = {
        key: value
        for key, value in {
...
def _hot_path_signal_score(value: Any) -> int:
    if isinstance(value, context_engine_store.Mapping):
        try:
...
def _hot_path_recomputed_readiness(
    *,
    ...
    routing_handoff = dict(compact_payload.get("routing_handoff", {})) if isinstance(compact_payload.get("routing_handoff"), context_engine_store.Mapping) else {}
    context_packet = dict(compact_payload.get("context_packet", {})) if isinstance(compact_payload.get("context_packet"), context_engine_store.Mapping) else {}
    ...
    retrieval_plan = dict(context_packet.get("retrieval_plan", {})) if isinstance(context_packet.get("retrieval_plan"), context_engine_store.Mapping) else {}
    packet_quality = packet_quality_codec.expand_packet_quality(
        dict(context_packet.get("packet_quality", {})) if isinstance(context_packet.get("packet_quality"), context_engine_store.Mapping) else {}
    )
    execution_profile = (
        dict(context_packet.get("execution_profile", {}))
        if isinstance(context_packet.get("execution_profile"), context_engine_store.Mapping)
        else {}
    )
    execution_signals = (
        dict(execution_profile.get("signals", {}))
        if isinstance(execution_profile.get("signals"), context_engine_store.Mapping)
        else {}
    )
    ...
    route_ready = context_engine_store.routing.grounded_write_execution_ready(
    ...
    native_spawn_ready = context_engine_store.routing.native_spawn_execution_ready(
        route_ready=route_ready,
        full_scan_recommended=bool(compact_payload.get("full_scan_recommended")),
        narrowing_required=bool(route.get("narrowing_required")),
        within_budget=within_budget,
...
def _compact_hot_path_routing_handoff(summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    claim_paths = context_engine_store._normalized_string_list(summary.get("claim_paths"))
    if claim_paths:
    ...
    execution_profile = _compact_hot_path_route_execution_profile(
        ...
        if isinstance(summary.get("odylith_execution_profile"), context_engine_store.Mapping)
        else {}
    )
...
def _compact_hot_path_route_execution_profile(profile: Mapping[str, Any]) -> dict[str, Any]:
    return context_engine_store.tooling_memory_contracts.compact_execution_profile_mapping(profile)

def _encode_hot_path_execution_profile(profile: Mapping[str, Any]) -> str:
    return context_engine_store.tooling_memory_contracts.encode_execution_profile_token(profile)

def _decode_hot_path_execution_profile(value: Any) -> dict[str, Any]:
    return context_engine_store.tooling_memory_contracts.compact_execution_profile_mapping(value)
...
def _compact_hot_path_payload_within_budget(
    *,
    ...
    optimization = (
        ...
        if isinstance(context_packet.get("optimization"), context_engine_store.Mapping)
        else {}
    )
    ...
    packet_budget = (
        ...
        if isinstance(context_packet.get("packet_budget"), context_engine_store.Mapping)
        else {}
    )
    ...
    return bool(context_packet) and not isinstance(payload.get("packet_metrics"), context_engine_store.Mapping) and packet_state in {
        "compact",
        "gated_ambiguous",
        "gated_broad_scope",
        "gated_low_signal",
...
def _synthesized_hot_path_execution_profile_from_context_packet(context_packet: Mapping[str, Any]) -> dict[str, Any]:
    ...
    governance = governance_signal_codec.expand_governance_signal(
        dict(route.get("governance", {})) if isinstance(route.get("governance"), context_engine_store.Mapping) else {}
    )
    selected_counts = context_engine_store._decode_compact_selected_counts(retrieval_plan.get("selected_counts"))
...
def _governance_changed_paths_shadow_redundant(
    *,
    ...
    route_payload = (
        ...
        if isinstance(context_packet_payload.get("route"), context_engine_store.Mapping)
        else {}
    )
    ...
    anchors_payload = (
        ...
        if isinstance(context_packet_payload.get("anchors"), context_engine_store.Mapping)
        else {}
    )
    anchor_changed = context_engine_store._normalized_string_list(anchors_payload.get("changed_paths"))
    if not anchor_changed:
        return False
    covered_paths = set(anchor_changed)
    covered_paths.update(context_engine_store._normalized_string_list(docs))
...
def _trim_common_hot_path_context_packet(
    *,
    ...
    route_payload = (
        ...
        if isinstance(compact.get("route"), context_engine_store.Mapping)
        else {}
    )
    ...
    anchors_payload = (
        ...
        if isinstance(compact.get("anchors"), context_engine_store.Mapping)
        else {}
    )
    if anchors_payload:
    ...
    execution_profile_payload = _compact_hot_path_route_execution_profile(
        ...
        if isinstance(compact.get("execution_profile"), context_engine_store.Mapping)
        else {}
    )
    ...
    retrieval_plan_payload = (
        ...
        if isinstance(compact.get("retrieval_plan"), context_engine_store.Mapping)
        else {}
    )
    if retrieval_plan_payload:
        ambiguity_class = str(retrieval_plan_payload.get("ambiguity_class", "")).strip()
        has_guidance_behavior_summary = isinstance(compact.get("guidance_behavior_summary"), context_engine_store.Mapping)
        retrieval_plan_payload.pop("selected_domains", None)
        selected_counts_payload = context_engine_store._decode_compact_selected_counts(
            retrieval_plan_payload.get("selected_counts")
        )
        if selected_counts_payload:
            compact_selected_counts = context_engine_store._encode_compact_selected_counts(selected_counts_payload)
        ...
        miss_recovery_payload = (
            ...
            if isinstance(retrieval_plan_payload.get("miss_recovery"), context_engine_store.Mapping)
            else {}
        )
    ...
    packet_quality_payload = (
        ...
        if isinstance(compact.get("packet_quality"), context_engine_store.Mapping)
        else {}
    )
    ...
    packet_budget_payload = (
        ...
        if isinstance(compact.get("packet_budget"), context_engine_store.Mapping)
        else {}
    )
...
def _compact_hot_path_narrowing_guidance(guidance: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if reason:
        compact["reason"] = (
            ...
            else context_engine_store._truncate_text(reason, max_chars=40)
        )
    suggestion_map = {
    ...
    suggested_inputs = [
        ...
        for text in context_engine_store._normalized_string_list(guidance.get("suggested_inputs"))[:1]
    ]
    if suggested_inputs:
    ...
    if (
        ...
        and anchors
        and len(anchors) == 1
        and context_engine_store._workstream_token(str(anchors[0].get("value", "")).strip())
    ):
    ...
    *,
    ...
    trimmed: dict[str, Any] = {
        key: (
            dict(value)
            if isinstance(value, context_engine_store.Mapping)
            else [dict(row) if isinstance(row, context_engine_store.Mapping) else row for row in value]
    ...
    context_packet_payload = (
        ...
        if isinstance(trimmed.get("context_packet"), context_engine_store.Mapping)
        else {}
    )
...
def _drop_redundant_hot_path_routing_handoff(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    routing_handoff = (
        ...
        if isinstance(compact.get("routing_handoff"), context_engine_store.Mapping)
        else {}
    )
    ...
    if any(
        ...
        if (
            ...
            and isinstance(compact.get("context_packet"), context_engine_store.Mapping)
        ):
