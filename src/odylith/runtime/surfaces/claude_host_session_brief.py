...
def _queue_refresh_if_briefs_stale(*, repo_root: Path) -> None:
    """Check if the global standup briefs are older than the staleness threshold.

    If stale, queue a background compass refresh. This runs at session start
    so the operator gets fresh briefs without manually refreshing. The refresh
    runs in the background and does not block session startup.
    """
    try:
