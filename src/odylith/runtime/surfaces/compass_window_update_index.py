...
def _finalize_candidate_buckets(
    *,
    ...
    host = _host()
    ordered = host._ordered_update_candidates
    updates: list[dict[str, str]] = []
    for bucket in (ordered(primary), ordered(secondary)):
...
def build_execution_update_index(
    ...
    *,
    ...
    host = _host()
    humanize = host._humanize_execution_event_summary
    sanitize = host._sanitize_digest_summary
    looks_generic = host._looks_generic_churn_summary
    signal_score = host._narrative_signal_score
    is_synthetic = host._is_synthetic_plan_execution_signal
    ...
    for event in window_events:
        ...
        normalized_summary = sanitize(summary)
        if not normalized_summary or looks_generic(normalized_summary):
            continue
        ...
        primary_bucket = kind in {"statement", "implementation", "decision", "commit"} and not is_synthetic(
            kind=kind,
...
def build_transaction_update_index(
    ...
    *,
    ...
    host = _host()
    split_source_vs_generated_files = host._split_source_vs_generated_files
    narrative_excerpt = host._narrative_excerpt
    humanize = host._humanize_execution_event_summary
    is_synthetic = host._is_synthetic_plan_execution_signal
    normalize_sentence = host._normalize_sentence
    local_change_headline = host._local_change_headline_phrase
    sanitize = host._sanitize_digest_summary
    looks_generic = host._looks_generic_churn_summary
    transaction_end_ts = host._transaction_end_ts
    signal_score = host._narrative_signal_score
    ...
    for row in window_transactions:
        ...
        source_files, generated_files = split_source_vs_generated_files(files)
        ...
        context = narrative_excerpt(str(row.get("context", "")).strip(), max_sentences=1, max_chars=220).strip()
        headline = narrative_excerpt(str(row.get("headline", "")).strip(), max_sentences=1, max_chars=220).strip()
        ...
        summary_lower = normalize_sentence(summary).lower()
        ...
        ts = transaction_end_ts(row)
        sort_ts = ts.timestamp() if isinstance(ts, dt.datetime) else 0.0
        candidate = {
            "kind": "transaction",
            "summary": normalized_summary,
            "_score": signal_score(normalized_summary, files=source_files),
            "_sort_ts": sort_ts,
        }
