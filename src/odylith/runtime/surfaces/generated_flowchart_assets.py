...
def _render_svg_groups(model: FlowchartModel, layout: FlowchartLayout) -> list[str]:
    ...
    for group_id, boxes in by_group.items():
        ...
        rendered.append(
            f'<text x="{x1 + 16}" y="{y1 + 22}" fill="{_STRUCTURE_LABEL_TEXT}" font-family="Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" font-size="12" font-weight="600">{html.escape(labels.get(group_id, group_id))}</text></g>'
        )
    return rendered
...
def _render_png(model: FlowchartModel, layout: FlowchartLayout) -> bytes:
    ...
    for node in model.nodes:
        ...
        for index, rendered_line in enumerate(box.lines[:4]):
            ...
            rect(bar_x, bar_y, bar_width, 4, _rgb(_STRUCTURE_LABEL_TEXT), _rgb(_STRUCTURE_LABEL_TEXT))
