...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- Empty/thin repos can receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write Product Intent Confirmation. In chat, show sectioned Markdown: Product story, State object, First complete path, Human actors, External systems, Internal product systems, assumptions, ambiguities, Proof boundary, and Confirm/Edit/Reject choices. Keep story/path/proof short, use bullets for lists and choices, and avoid wall of prose, code ticks, or decorative bold. Before confirmation, no backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or code. After confirmation, write the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes to `.odylith/runtime/greenfield/confirmed-intent.json`, validates, runs the bounded, provider-free post-confirm repair loop, writes records after the final manifest passes, and refreshes views. Do not inspect Odylith source. Do not ask the operator to inspect JSON, ask for second confirmation, hand-author/repair proposal JSON, narrate parser/schema retries, stop after repairable quality issues, or run confirmed create from a thin prompt without `--intent-file`. Surface only the created-record summary or final blockers.",
