...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- Empty or thin consumer repos can still receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt \"<request>\"` for a no-write Product Intent Confirmation. In chat, show sectioned Markdown: Product story, State object, First complete path, Human actors, External systems, Internal product systems, assumptions, ambiguities, Proof boundary, and Confirm/Edit/Reject as three separate bullet lines. Keep story/path/proof as short paragraphs, use bullets for lists and the Confirm/Edit/Reject choices, and avoid wall of prose, code ticks, or decorative bold. Before confirmation, no backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or code. After confirmation, write the same visible confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes to `.odylith/runtime/greenfield/confirmed-intent.json`, validates, writes records, and refreshes views. Do not inspect Odylith source. Do not ask the operator to inspect JSON, ask for a second confirmation, hand-author or repair proposal JSON, narrate parser/schema retries, or run confirmed create from a thin prompt without `--intent-file`. Surface only the created-record summary or blockers.",
