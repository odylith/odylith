...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- Empty/thin repos: run `odylith greenfield propose --prompt \"<request>\"`; in chat show sectioned Markdown: Product story, State object, First complete path, Proof boundary, and `## Choose one command`. The command block must show `Start your reply with exactly one command: **CONFIRM**, **EDIT**, or **REJECT**.`, then visually separate sections headed **Command: `CONFIRM`**, **Command: `EDIT`**, and **Command: `REJECT`** plus `Reply starts with: CONFIRM`, `Reply starts with: EDIT`, and `Reply starts with: REJECT`. Save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`; run `odylith greenfield compile-transaction --intent-file .odylith/runtime/greenfield/confirmed-intent.md --output .odylith/runtime/greenfield/product-create-transaction.v1.json --release 0.0.1`; Odylith may write `.odylith/runtime/greenfield/confirmed-intent.json`, quality-gates, and hashes the ProductCreateTransaction. The first **CONFIRM** compiles and hash-gates the transaction before any governed records are written. The hash confirmation runs `odylith greenfield create --transaction-file .odylith/runtime/greenfield/product-create-transaction.v1.json --transaction-hash <hash> --confirm`; create verifies hash and writes atomically. Treat **EDIT** corrections after the command as new evidence and rebuild. Treat **REJECT** as stop with no governed records. Do not inspect Odylith source, hand-author/repair proposal JSON, narrate parser/schema retries, ask for a second confirmation around uncompiled Markdown, or run create without a verified ProductCreateTransaction. Surface only the product story created-record summary or final blockers without a second confirmation.",
