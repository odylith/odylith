...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- Empty or thin consumer repos can still receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt \"<request>\"` for a no-write Product Intent Confirmation. In chat, narrate sectioned Markdown: Product story, State object, First complete path, Human actors, External systems, Internal product systems, Critical assumptions, Ambiguities, Proof boundary, and Confirm/Edit/Reject. Keep story/path/proof as short paragraphs, use bullets for lists, and avoid a wall of prose, code ticks, or decorative bold around domain nouns. Before confirmation, no backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or code. After confirmation, write that same visible confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes it into `.odylith/runtime/greenfield/confirmed-intent.json`, validates, writes governed records, and refreshes readable views. Optional audit uses `odylith greenfield propose --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm-intent --format json`. Do not inspect Odylith source. Do not ask the operator to inspect JSON, ask for a second confirmation, hand-author or repair proposal JSON, narrate parser/schema retries, or run confirmed create from a thin prompt without `--intent-file`. Surface only the created-record summary or blockers. No canned domain families, scaffolds, or code before product gates.",
