...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- Empty/thin repos: run `odylith greenfield propose --prompt \"<request>\"`; show sectioned Markdown: Product story, State object, First complete path, Proof boundary, and `## Choose one command`; avoid wall of prose and keep the product story concrete. Show `Start your reply with exactly one command: **CONFIRM**, **EDIT**, or **REJECT**.`, **Command: `CONFIRM`**, **Command: `EDIT`**, **Command: `REJECT`**, and `Reply starts with: CONFIRM`, `Reply starts with: EDIT`, `Reply starts with: REJECT`. Save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`; run `odylith greenfield compile-transaction --intent-file .odylith/runtime/greenfield/confirmed-intent.md --output .odylith/runtime/greenfield/product-create-transaction.v1.json --release 0.0.1`; Odylith may write `.odylith/runtime/greenfield/confirmed-intent.json` and seals the quality-gated ProductCreateTransaction before governed writes. CONFIRM runs `odylith greenfield create --transaction-file .odylith/runtime/greenfield/product-create-transaction.v1.json --transaction-hash <hash> --confirm`; create verifies the compiler receipt, hash, and repo preconditions, writes only sealed bytes under rollback guard, and validates readback. EDIT adds evidence and rebuilds; REJECT stops with no writes. Do not inspect Odylith source, hand-author/repair proposal JSON, or narrate parser/schema retries. Do not ask operator to inspect proposal JSON or for a second confirmation; do not create without a verified transaction. Surface only the created-record summary or final blockers.",
