...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- Empty or thin consumer repos can still receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt \"<request>\"` for the no-write Product Intent reasoning request, then write the short Product Intent Confirmation in chat from live reasoning. The confirmation must explain the product story, actors, systems, critical assumptions, material ambiguities, and proceed/edit/reject gate. Do not generate backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or prompt-only `greenfield create` records before the operator confirms that intent. After confirmation, use `odylith greenfield propose --confirm-intent` for the proposal schema/proof contract; the host authors the proposal JSON from the confirmed product shape, and `odylith greenfield apply --repo-root . --proposal-file odylith-greenfield-proposal.json --confirm --release 0.0.1` validates it, runs the Tribunal write gate, writes governed records, refreshes readable views, and hands off without exposing tool internals as product requirements. Do not use canned domain families, scaffolds, or code before the product gates are accepted. When the CLI returns proposal stdout directly, do not hide it behind collapsed tool output or replace it with a thin host-written summary; surface the actual confirmation or proposal content that matters.",
