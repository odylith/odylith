...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- Empty/thin repos: run `odylith greenfield propose --prompt \"<request>\"`; in chat show sectioned Markdown: Product story, State object, First complete path, Proof boundary, and Confirm/Edit/Reject. Keep bullets where useful and avoid wall of prose. Save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`; run `odylith greenfield compile-transaction --intent-file .odylith/runtime/greenfield/confirmed-intent.md --output .odylith/runtime/greenfield/product-create-transaction.v1.json --release 0.0.1`; Odylith may write `.odylith/runtime/greenfield/confirmed-intent.json`, quality-gates, and hashes the ProductCreateTransaction. Confirm with `odylith greenfield create --transaction-file .odylith/runtime/greenfield/product-create-transaction.v1.json --transaction-hash <hash> --confirm`; create verifies hash and writes atomically. Do not inspect Odylith source, hand-author/repair proposal JSON, narrate parser/schema retries, ask for a second confirmation, or run create without a verified ProductCreateTransaction. Surface only the product story created-record summary or final blockers without a second confirmation.",
