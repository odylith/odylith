...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- Empty or thin consumer repos can still receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt \"<request>\"` for the no-write Product Intent Confirmation, then narrate the product story, actors, systems, assumptions, ambiguities, and Confirm/Edit/Reject choices. Before confirmation, do not write backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or code. After confirmation, write that same visible confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes the Markdown into `.odylith/runtime/greenfield/confirmed-intent.json`, builds and validates the governed proposal, runs the Tribunal write gate, writes records, and refreshes readable views. `odylith greenfield propose --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm-intent --format json` is optional audit output only when explicitly requested. Do not inspect Odylith source for schema fields. Do not ask the operator to inspect JSON or confirm a second time, hand-author or repair proposal JSON, narrate parser/schema retries, or run confirmed create from a thin prompt without `--intent-file`. No canned domain families, scaffolds, or code before product gates. Surface only the human-readable created-record summary or final validation/Tribunal blockers.",
