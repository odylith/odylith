...
def update_root_guidance_files(*, repo_root: Path, install_active: bool, repo_role: str) -> None:
    ...
    if (
        ...
        and root_agents.is_file()
        and "Odylith is a product repo, not a host repo." in root_agents.read_text(encoding="utf-8")
    ):
        # Tracked product guidance is source truth, not an installed asset.
        return
    for path in existing_top_level_guidance_paths(repo_root=root):
...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- In commentary, keep startup, fallback, routing, and packet-selection internals implicit. Describe progress in task terms like the exact file/workstream, the bug under test, or the validation in flight. Do not surface routine `odylith start`, `odylith context`, or `odylith query` commands in progress updates; never prefix commentary with control-plane receipt labels. Keep normal commentary task-first and human. Reserve `Odylith Insight:`, `Odylith History:`, or `Odylith Risks:` for rare high-signal moments.",
            ...
            "- Empty/thin repos: use the active host model for semantic reasoning and Odylith for deterministic verification. Run `odylith greenfield semantic-intent-request --repo-root . --prompt \"<request>\"`, author its source-cited typed packet with the simplest evidence-supported reasoning mechanism, independently challenge every material claim, then run the returned `greenfield propose ... --semantic-intent-file ...` invocation. The outcome and proof laws are fixed; the authoring mechanism remains provisional. A citation proves custody, not entailment. Odylith deterministically verifies typed endpoints, graph completeness, and the full staged ProductCreateTransaction without semantic regexes or repair retries, then renders the sole visible confirmation view. The confirmation Markdown is a human view, never product truth. CONFIRM commits the exact reviewed, hash-bound ProductCreateTransaction; EDIT treats corrections as new evidence and uses `semantic-intent-request --supersedes-hash <hash> --edit \"<corrections>\"` to rebuild from sealed prior evidence; REJECT writes nothing. `odylith greenfield create --repo-root . --transaction-file .odylith/runtime/greenfield/pending/<hash>/product-create-transaction.v1.json --transaction-hash <hash> --confirm` does not parse or generate product content after CONFIRM; it only verifies the sealed receipt, hash, and preconditions, writes the exact sealed bytes under rollback guard, then validates readback. Never add prompt-specific parsing rules or hand-author proposal JSON. Surface only the final transaction, created-record summary, or blocker.",
            ...
            "- A concrete decision or proof-boundary prompt may get one short `Odylith Assist:`. When explicit intervention feedback asks for more frequent Assist, treat that as an earned cadence: surface concise user-facing decision, risk, proof, verified-result, and substantive-continuation beats while keeping bare acknowledgements, routine chatter, and internal execution details out. Never add Assist merely because Odylith ran. Lead with the user win, changed IDs, the `odylith_off` edge, and concrete counts, deltas, or validation outcomes. Generic receipts are not premium interventions. Silence is better than filler.",
            ...
            "- Governance-learning is mandatory across maintainer, source-local, installed consumer repos, Codex, Claude Code, and bundled/generated guidance. Durable errors, failed mechanisms, bad artifacts, semantic drift, gate failures, decisions, validation results, and release-risk learning update Casebook, Radar or plans, Registry, Atlas, or Compass before continuing, committing, building, releasing, or claiming completion. Before fixes, search existing truth first, read prior failed mechanisms and guardrails, avoid duplicate Casebook bugs, use Odylith CLI paths where they exist, and never treat chat/final summaries as durable truth.",
            ...
            "- Treat AI slop as a regression. This covers any codebase or project surface across consumer/maintainer lanes, Codex/Claude, all languages, and every code, prompt, hook, config, template, or generated asset. Treat the slop class, not the syntax, as the thing to ban. Move ownership, not just files; partial shared-kernel adoption is still incomplete; prose-only hardening is incomplete; if smell remains, the pass is incomplete. Claims require fresh behavior proof for the touched slice and a fresh structural inventory for the claimed scope; headless browser matrix requires normal, empty/fallback, and degraded or error states. Full rule: `odylith/agents-guidelines/ANTI_SLOP_AND_DECOMPOSITION.md`; use `odylith/skills/odylith-code-hygiene-guard/SKILL.md` when quality pressure is high.",
