...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- In coding-agent commentary, keep startup, fallback, routing, and packet-selection internals implicit. Describe progress in task terms like the exact file/workstream, the bug under test, or the validation in flight. Do not surface routine `odylith start`, `odylith context`, or `odylith query` commands in progress updates, and never prefix commentary with control-plane receipt labels.",
            "- Keep normal commentary task-first and human. Reserve `Odylith Insight:`, `Odylith History:`, or `Odylith Risks:` for rare high-signal moments.",
            ...
            "- For Codex checkpoint hooks, keep the full Observation, Proposal, and Assist bundle in hidden developer context for continuity, and surface the earned Observation/Proposal note when the host renders hook output. Claude direct-edit and Bash PostToolUse hooks stay silent on success and emit only compact failure/skipped-refresh status; Claude Stop is memory/logging only, not a fallback closeout or live-note recovery lane.",
            ...
            "- CLI-first is non-negotiable: default to the nearest `AGENTS.md`, the repo-local launcher, and truthful `odylith ... --help` for routine backlog, technical-plan, bug, spec, component, and diagram upkeep. Use the actual CLI family: `odylith backlog ...`, `odylith governance ...`, `odylith validate plan-* ...`, `odylith bug ...`, `odylith component ...`, `odylith registry ...`, `odylith atlas ...`, and `odylith compass ...`. Do not hand-edit governed files where a CLI exists. `odylith plan --help` is a read-only command guide; do not invent `odylith plan create/edit` flows or probe `odylith/technical-plans/source/`.",
