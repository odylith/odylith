...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- Empty or thin consumer repos can still receive proposal-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt \"<request>\"` to review the canonical apply-ready host-reasoning evidence/schema contract, including host-authored Atlas Mermaid source. Use `odylith greenfield create --repo-root . --prompt \"<request>\" --release 0.0.1 --confirm` after confirmation so Odylith owns proposal construction, validation, Tribunal, governed writes, refresh, and handoff. Do not hand-author proposal JSON.",
