...
def repo_root_claude_source() -> str:
    return "\n".join(
        [
            ...
            "- First-match demo route: if the user says `Odylith, show me what you can do` or asks what Odylith can do for this repo, use the advisory `odylith show` demo. Print its stdout only, then stop the turn without extra shell probes. Do not run install, status, intervention, launcher diagnostics, or sample-app creation first. If Odylith is not installed in the current folder, say that directly.",
...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- In coding-agent commentary, keep startup, fallback, routing, and packet-selection internals implicit. Describe progress by the exact file/workstream, bug, or validation in flight. If startup needs narrowing but work can continue, do not narrate it; never say `Startup fell back`. Do not surface routine `odylith start`, `odylith context`, or `odylith query`, never prefix commentary with control-plane receipt labels, and reserve `Odylith Insight:`, `Odylith History:`, or `Odylith Risks:` for rare high-signal moments.",
            ...
            "- Empty/thin repos can receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write Product Intent Confirmation. In chat, show sectioned Markdown: Product story, State object, First complete path, Human actors, External systems, Internal product systems, assumptions, ambiguities, Proof boundary, and Confirm/Edit/Reject choices. Keep story/path/proof short, use bullets, and avoid wall of prose, code ticks, or decorative bold. Before confirmation: no backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or code. After confirmation, write the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes to `.odylith/runtime/greenfield/confirmed-intent.json`, validates, runs the bounded, provider-free post-confirm repair loop, writes records only after the final manifest passes, and refreshes views. Do not inspect Odylith source, hand-author/repair proposal JSON, narrate parser/schema retries, stop after repairable quality issues, or run confirmed create from a thin prompt without `--intent-file`. Do not ask the operator to inspect JSON or for second confirmation. Surface only the created-record summary or final blockers.",
            ...
            "- At closeout or visible-intervention recovery, add at most one short `Odylith Assist:` or `**Odylith Assist:**` line only when it helps. Normal non-passthrough prompts do not get an Assist line by default; do not add Assist just because Odylith ran. Lead with the user win, updated governance IDs when changed, the `odylith_off` edge when supported, and concrete observed counts, measured deltas, validation outcomes, or a chat-visibility complaint. Generic receipts are not premium interventions; stay silent unless a concrete Observation, Proposal, validation result, or visibility failure earns the note.",
            ...
            "- Treat backlog/workstream, plan, Registry, Atlas, Casebook, Compass, and session upkeep as one grounded workflow; search existing workstream, plan, bug, component, diagram, and recent Compass truth first. Create new work only for genuinely new repo-owned slices; consumer-found Odylith issues need a maintainer-ready packet.",
            "- Governance-learning is mandatory across maintainer, pinned dogfood, detached `source-local`, installed consumer repos, Codex, Claude Code, and bundled/generated guidance. Durable errors, escaped defects, failed mechanisms/simulations, bad artifacts, semantic drift, quality-gate failures, latency breaches, architecture decisions, validation results, and release-risk learning must update Casebook, Radar or technical plans, Registry, Atlas, or Compass before continuing, committing, building, releasing, or claiming completion. Before fixing any bug, search existing Casebook and related governance artifacts, read prior failed mechanisms, failed fix attempts, and guardrails, and do not repeat a fix path that has already failed. Search existing truth first, avoid duplicates, use CLI paths where they exist, and do not treat chat memory or final summaries as durable truth.",
            ...
            "- Treat AI slop as a regression across consumer/maintainer lanes, Codex/Claude, all languages, and every code, prompt, hook, config, template, generated asset, or project surface. Treat the slop class, not the syntax, as the thing to ban. Move ownership, not just files; partial shared-kernel adoption and prose-only hardening are incomplete. Repo-wide claims require fresh behavior proof plus structural inventory; browser surfaces require normal, empty/fallback, and degraded/error headless checks. Full rule: `odylith/agents-guidelines/ANTI_SLOP_AND_DECOMPOSITION.md`; use `odylith/skills/odylith-code-hygiene-guard/SKILL.md` when quality pressure is high.",
...
def customer_bootstrap_claude_source() -> str:
    return "\n".join(
        [
            ...
            "- Apply the shared governance-learning rule here as well: durable errors, decisions, proof checkpoints, and contract changes must reach Casebook, Radar or plans, Registry, Atlas, or Compass before closeout. Before fixing a bug, search Casebook and related governance truth, read prior failed mechanisms, failed fix attempts, and guardrails, do not repeat a fix path that already failed, and capture new mechanism-level learning.",
            "- First-match help route: if the user says `Odylith, help`, use the CLI help surface and print stdout only. Do not run install, status, intervention, or launcher diagnostics first.",
            "- First-match demo route: if the user says `Odylith, show me what you can do` or asks what Odylith can do for this repo, use the advisory `odylith show` demo. Print its stdout only, then stop the turn without extra shell probes. Do not run install, status, intervention, launcher diagnostics, or sample-app creation first. If Odylith is not installed in the current folder, say that directly.",
            "- Capability inventory route: if the user asks to list Odylith capabilities, engines, product architecture, or the capability map, run `odylith capabilities` and print stdout only. Do not infer the taxonomy from `odylith --help`, `odylith show`, Claude Code capability prose, or any host-model surface.",
