...
def customer_bootstrap_guidance() -> str:
    return "\n".join(
        [
            ...
            "- Empty/thin repos: run `odylith greenfield propose --repo-root . --prompt \"<request>\"`. Odylith compiles typed evidence into a fully quality-gated staged ProductCreateTransaction before it presents the only command rail. CONFIRM commits the shown hash-bound package with `CONFIRM <hash>`; EDIT treats corrections as new evidence and rebuilds with `EDIT <hash> <corrections>`; REJECT writes nothing with `REJECT <hash>`. Ask one focused question only for material uncertainty; carry other gaps as visible assumptions. Markdown is a view, never product truth. `odylith greenfield create --repo-root . --transaction-file .odylith/runtime/greenfield/pending/<hash>/product-create-transaction.v1.json --transaction-hash <hash> --confirm` does not parse or generate product content after CONFIRM; it only verifies the sealed receipt, hash, and preconditions, writes the exact sealed bytes under rollback guard, then validates readback. Do not inspect source, hand-author/repair proposal JSON, narrate parser/schema retries, or request a second confirmation. Surface only the final transaction, created-record summary, or blocker.",
