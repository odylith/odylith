...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for a compact project-first/product-first interpretation, direction choices, non-goals, first release ambition, and proof gates; print the proposal stdout directly in chat instead of leaving it only in collapsed tool output or replacing it with a host-written action menu. Confirmed writes use `odylith greenfield create --repo-root . --prompt \"<request>\" --release 0.0.1 --confirm` after validation and the Tribunal write gate. Do not hand-author proposal JSON or code before the product gates are accepted.",
        ...
        (
            ...
            if product_repo
            else "- In coding-agent commentary, keep startup, fallback, routing, and packet-selection internals implicit. Describe progress in task terms like the exact file/workstream, the bug under test, or the validation in flight. If startup needs narrowing but work can continue, do not narrate it, and never say `Startup fell back`. Do not surface routine `odylith start`, `odylith context`, or `odylith query` commands in progress updates, and never prefix commentary with control-plane receipt labels. Mention Odylith during the work only when the user explicitly asks for the command, a real blocker requires it, or a consumer-versus-maintainer lane distinction matters."
        ),
