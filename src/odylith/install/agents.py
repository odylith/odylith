...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        (
            "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write, project-first Product Intent Confirmation. In chat, show sectioned Markdown: title, Product story, State object, First complete path, actors, systems, assumptions, ambiguities, Proof boundary, and Confirm/Edit/Reject. Use short paragraphs and bullets; no wall of prose, code ticks, or decorative bold around domain nouns. Before confirmation, no backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or source. After confirmation, save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes it into `.odylith/runtime/greenfield/confirmed-intent.json`, runs the bounded, provider-free post-confirm repair loop, and writes records after the final manifest passes. JSON audit: `odylith greenfield propose --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm-intent --format json` on request. Do not inspect Odylith source, hand-author/repair proposal JSON, or narrate parser/schema retries. Do not stop after repairable quality issues. Do not ask the operator to inspect JSON or for second confirmation. Surface created-record summary/final blockers."
            if product_repo
            else "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write, project-first Product Intent Confirmation. In chat, show sectioned Markdown: title, Product story, State object, First complete path, actors, systems, assumptions, ambiguities, Proof boundary, and Confirm/Edit/Reject bullet lines. Use short paragraphs and bullets; no wall of prose, code ticks, decorative bold, proposal JSON, or source before confirmation. After confirmation, save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes it into `.odylith/runtime/greenfield/confirmed-intent.json`, runs the bounded, provider-free post-confirm repair loop, and writes records after the final manifest passes. Do not inspect Odylith source, repair proposal JSON, narrate parser/schema retries, stop after repairable quality issues, ask for JSON inspection, or ask for a second confirmation. Surface only the created-record summary or final blockers."
        ),
        ...
        "- Governance-learning is a default across maintainer, pinned dogfood, detached `source-local`, installed consumer repos, Codex, Claude Code, and bundled/generated guidance. For every durable error, escaped defect, failed mechanism, failed simulation, live repro, bad generated artifact, semantic drift, quality-gate failure, latency breach, architecture decision, validation result, or release-risk learning, update the appropriate Odylith governance artifact before continuing, committing, building, releasing, or claiming completion: Casebook for defects and regressions, Radar or technical plans for planned work, Registry for component-contract changes, Atlas for changed flow/topology/projection boundaries, and Compass for execution decisions and proof checkpoints. Before fixing any bug, search existing Casebook and related governance artifacts, read prior failed mechanisms, failed fix attempts, and guardrails, and do not repeat a fix path that has already failed. Use Odylith CLI paths first, search existing governance truth before creating new records, and never treat chat memory or final-answer summaries as durable governance truth.",
