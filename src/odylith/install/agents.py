...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        (
            "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write, project-first Product Intent Confirmation using sectioned Markdown. Before confirmation: no source/JSON/backlog/Registry/Atlas/release/validation writes. After confirmation, save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes it into `.odylith/runtime/greenfield/confirmed-intent.json`, runs the bounded, provider-free post-confirm repair loop, and writes records only after the final manifest passes. Do not inspect source/repair JSON, narrate retries, stop after repairable issues, or seek JSON review/second confirmation."
            if product_repo
            else "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write, project-first Product Intent Confirmation using sectioned Markdown. Before confirmation: no prose/JSON/source/backlog/Registry/Atlas/release/validation writes. After confirmation, save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes it into `.odylith/runtime/greenfield/confirmed-intent.json`, runs the bounded, provider-free post-confirm repair loop, and writes records only after the final manifest passes. Do not inspect source/repair JSON, narrate retries, stop after repairable issues, or seek JSON review/second confirmation."
        ),
