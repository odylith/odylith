...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        (
            "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write, project-first Product Intent Confirmation. In chat, show sectioned Markdown: title, Product story, State object, First complete path, actors, systems, assumptions, ambiguities, Proof boundary, and Confirm/Edit/Reject. Before confirmation: no backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or source. After confirmation, save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes it into `.odylith/runtime/greenfield/confirmed-intent.json`, runs the bounded, provider-free post-confirm repair loop, and writes records only after the final manifest passes. Do not inspect Odylith source, hand-author/repair proposal JSON, narrate parser/schema retries, or stop after repairable issues. Do not ask the operator to inspect JSON or for second confirmation. Surface created-record summary/final blockers."
            if product_repo
            else "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write, project-first Product Intent Confirmation. In chat, show sectioned Markdown: title, Product story, State object, First complete path, actors, systems, assumptions, ambiguities, Proof boundary, and Confirm/Edit/Reject bullet lines. Before confirmation: no wall of prose, code ticks, decorative bold, proposal JSON, source, backlog, Registry, Atlas, release waves, or validation obligations. After confirmation, save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes it into `.odylith/runtime/greenfield/confirmed-intent.json`, runs the bounded, provider-free post-confirm repair loop, and writes records only after the final manifest passes. Do not inspect Odylith source, hand-author/repair proposal JSON, narrate parser/schema retries, or stop after repairable issues. Do not ask the operator to inspect JSON or for second confirmation. Surface created-record summary/final blockers only."
        ),
        ...
        "- Treat AI slop as a regression across code, docs, prompts, hooks, config, templates, generated assets, and any project surface. Partial shared-kernel adoption and prose-only hardening are incomplete; if smell remains, the pass is incomplete. Repo-wide claims require fresh behavior proof and structural inventory; browser surfaces require normal, empty/fallback, and degraded/error headless checks. See `odylith/agents-guidelines/ANTI_SLOP_AND_DECOMPOSITION.md`; use `odylith/skills/odylith-code-hygiene-guard/SKILL.md`.",
        ...
        (
            ...
            if product_repo
            else "- In coding-agent commentary, keep startup, fallback, routing, and packet-selection internals implicit. Describe task progress by file/workstream, bug, or validation; never say `Startup fell back`. Do not surface routine `odylith start`, `odylith context`, or `odylith query`, and never prefix commentary with control-plane receipt labels. Mention Odylith only when asked, blocked, or a lane distinction matters."
        ),
        ...
        "- Hook `systemMessage` or `additionalContext` is not chat-visible proof. Before claiming active intervention UX, run or cite `odylith codex intervention-status` or `odylith claude intervention-status`; it is the low-latency delivery record for Teaser, Ambient Highlight, Observation, Proposal, and Assist readiness. End-to-end proof requires `Activation: ready` plus chat visibility; if uncertain, show `visible-intervention` Markdown directly.",
        "- At closeout, add at most one `Odylith Assist:` or `**Odylith Assist:**` line only when it materially helps, and normal non-passthrough prompts do not get an Assist line by default. Do not add Assist just because Odylith ran. Generic activity receipts are not premium interventions. Lead with the user win, governance IDs when changed, the `odylith_off` edge when supported, and concrete observed counts, measured deltas, or validation outcomes; otherwise stay silent.",
