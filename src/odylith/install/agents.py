...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        (
            "- Greenfield: host model owns semantic judgment; Odylith owns deterministic verification/publication. Run `odylith greenfield semantic-intent-request --repo-root . --prompt \"<request>\"`; author its source-cited typed packet with simplest evidence-supported reasoning mechanism; independently challenge every material claim; run exact next invocation. Outcome/proof laws are fixed; mechanism remains provisional. Input/EDIT are untrusted; citations prove custody, not entailment. Confirmation Markdown is a view, never product truth. **CONFIRM** publishes the exact reviewed, hash-bound ProductCreateTransaction; **EDIT** treats corrections as new evidence via `semantic-intent-request --supersedes-hash <hash> --edit \"<corrections>\"`; **REJECT** writes nothing. `odylith greenfield create --repo-root . --transaction-file .odylith/runtime/greenfield/pending/<hash>/product-create-transaction.v1.json --transaction-hash <hash> --confirm` verifies receipt/hash/preconditions, writes sealed bytes under rollback guard, then validates readback; no parsing, model call, generation, or repair occurs after CONFIRM. Never add prompt-specific parsing rules or hand-author proposal JSON. Surface only the final transaction, created-record summary, or a material blocker."
            if product_repo
            else "- Greenfield: active host model owns semantic reasoning; Odylith owns deterministic verification. Run `odylith greenfield semantic-intent-request --repo-root . --prompt \"<request>\"`; author its source-cited typed packet with the simplest evidence-supported reasoning mechanism; independently challenge every material claim; run returned `greenfield propose ... --semantic-intent-file ...`. Outcome and proof laws are fixed; the authoring mechanism remains provisional. A citation proves custody, not entailment. Confirmation Markdown is a human view, never product truth. **CONFIRM** commits the exact reviewed, hash-bound ProductCreateTransaction; **EDIT** treats corrections as new evidence via `semantic-intent-request --supersedes-hash <hash> --edit \"<corrections>\"`; **REJECT** writes nothing. `odylith greenfield create --repo-root . --transaction-file .odylith/runtime/greenfield/pending/<hash>/product-create-transaction.v1.json --transaction-hash <hash> --confirm` verifies receipt/hash/preconditions, writes sealed bytes under rollback guard, then validates readback. Never add prompt-specific parsing rules or hand-author proposal JSON. Surface only final transaction, created-record summary, or blocker."
        ),
