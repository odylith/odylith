...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write Product Intent Confirmation. In chat, write the product story, actors, systems, assumptions, ambiguities, first path, proof boundary, and Confirm/Edit/Reject next step. Before confirmation, no backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or create writes. After confirmation, write that same visible confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes that Markdown into `.odylith/runtime/greenfield/confirmed-intent.json` and builds the governed proposal from the accepted narrative. Optional JSON audit uses `odylith greenfield propose --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm-intent --format json` only when explicitly requested; it is not the normal write path. Do not inspect Odylith source, Python modules, local examples, `.odylith`, or bundle assets to discover schemas. Do not hand-author, switch to, or repair proposal JSON after confirmation. Do not narrate parser/schema retries or intermediate create-shape failures in operator chat. Do not ask the operator to inspect JSON or confirm a second time. Surface only the created-record summary or final blockers.",
