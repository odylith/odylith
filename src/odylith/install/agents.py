...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write Product Intent Confirmation. In chat, show product story, actors, systems, assumptions, ambiguities, first path, proof boundary, and Confirm/Edit/Reject. Before confirmation, no backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or writes. After confirmation, save the same visible confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes it into `.odylith/runtime/greenfield/confirmed-intent.json` and builds governed records. Optional JSON audit uses `odylith greenfield propose --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm-intent --format json` only when explicitly requested. Do not inspect Odylith source, Python modules, local examples, `.odylith`, or bundle assets to discover schemas. Do not hand-author, switch to, or repair proposal JSON after confirmation. Do not narrate parser/schema retries or intermediate create-shape failures. Do not ask the operator to inspect proposal JSON or confirm a second time. Surface only the created-record summary or final blockers.",
