...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for a no-write Product Intent Confirmation request. In chat, write it from live reasoning: product story, actors, systems, assumptions, ambiguities, and gate. Do not generate backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or prompt-only `greenfield create` records before confirmation. After confirmation, use `odylith greenfield propose --confirm-intent` for schema/proof, author the project-specific proposal JSON from intent, then run `odylith greenfield apply --repo-root . --proposal-file odylith-greenfield-proposal.json --confirm`; apply validates, Tribunal-gates, and writes governed records. No canned domain families, scaffolds, hand-waved templates, or code. If CLI returns proposal stdout directly, do not hide it behind collapsed tool output or replace it with a host-written summary.",
        ...
        "- For guidance or discipline pressure, use `odylith validate guidance-behavior --repo-root .`, quick guidance/discipline benchmarks, and `odylith discipline status/check/explain`; Discipline hot paths must not call host models, providers, subagents, broad scans, full validation, or projection expansion.",
        ...
        "- Hook `systemMessage` or `additionalContext` is not chat-visible proof. Before claiming active intervention UX, run or cite `odylith codex intervention-status` or `odylith claude intervention-status`; it is the low-latency delivery record for Teaser, Ambient Highlight, Observation, Proposal, and Assist readiness. End-to-end proof requires `Activation: ready` plus chat visibility. If uncertain, run `odylith codex visible-intervention` or `odylith claude visible-intervention`.",
