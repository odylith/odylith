...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        (
            ...
            if product_repo
            else "- Empty/thin prompts use `odylith greenfield propose --repo-root . --prompt \"<request>\"`. Odylith treats input as evidence, builds typed facts, repairs and quality-gates the full staged ProductCreateTransaction, then shows one hash-bound rail. **CONFIRM** commits that package with `CONFIRM <hash>`; **EDIT** adds evidence and rebuilds with `EDIT <hash> <corrections>`; **REJECT** stops with no writes using `REJECT <hash>`. Material uncertainty gets one focused question before the rail; non-material gaps become visible assumptions. Pre-confirm staging is allowed, but Markdown is a view, never product truth. `odylith greenfield create --repo-root . --transaction-file .odylith/runtime/greenfield/pending/<hash>/product-create-transaction.v1.json --transaction-hash <hash> --confirm` verifies receipt, hash, and repo preconditions, writes sealed bytes under rollback guard, and validates readback. Do not inspect Odylith source or hand-author/repair proposal JSON. Do not narrate parser/schema retries or request a second confirmation. Surface only the final transaction, created-record summary, or blocker."
        ),
