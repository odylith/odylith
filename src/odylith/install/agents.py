...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        (
            "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for a no-write, project-first Product Intent Confirmation. In chat, show sectioned Markdown: title, Product story, State object, First complete path, Human actors, External systems, Internal product systems, Critical assumptions, Ambiguities, Proof boundary, and Confirm/Edit/Reject as separate bullet lines. Use short story/path/proof paragraphs and bullets for actors, systems, assumptions, ambiguities, and choices; no wall of prose, code ticks, or decorative bold around domain nouns. Before confirmation, no backlog, Registry, Atlas, release waves, validation obligations, proposal JSON, or source. After confirmation, save that visible confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes it into `.odylith/runtime/greenfield/confirmed-intent.json` and builds governed records. JSON audit: `odylith greenfield propose --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm-intent --format json` on request. Do not inspect Odylith source, hand-author/repair proposal JSON, or narrate parser/schema retries. Do not ask the operator to inspect JSON or for second confirmation. Surface created-record summary/blockers."
            if product_repo
            else "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write, project-first Product Intent Confirmation. In chat, show sectioned Markdown: title, Product story, State object, First complete path, actors, systems, assumptions, ambiguities, Proof boundary, and Confirm/Edit/Reject as three separate bullet lines. Use short paragraphs and bullets; no wall of prose, code ticks, decorative bold, proposal JSON, or source before confirmation. After confirmation, save the same visible Product Intent Confirmation to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`; Odylith normalizes it into `.odylith/runtime/greenfield/confirmed-intent.json` before it builds governed records. Do not inspect Odylith source, repair proposal JSON, narrate parser/schema retries, ask for JSON inspection, or ask for a second confirmation. Surface only the created-record summary or final blockers."
        ),
