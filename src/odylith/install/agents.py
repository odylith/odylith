...
def managed_block(*, repo_role: str = "consumer_repo") -> str:
    ...
    lines = [
        ...
        (
            ...
            if product_repo
            else "- Empty/thin prompts route to `odylith greenfield propose --repo-root . --prompt \"<request>\"` for no-write Product Intent Confirmation. In chat, show title, Product story, State object, First complete path, actors, systems, assumptions, ambiguities, proof boundary, and Confirm/Edit/Reject as three separate bullet lines. Use short paragraphs and bullets; no wall of prose, code ticks, decorative bold, proposal JSON, or source before confirmation. After confirmation, save it to `.odylith/runtime/greenfield/confirmed-intent.md`, then run `odylith greenfield create --repo-root . --prompt \"<request>\" --intent-file .odylith/runtime/greenfield/confirmed-intent.md --confirm --release 0.0.1`. Do not inspect Odylith source, repair proposal JSON, narrate parser/schema retries, ask for JSON inspection, or ask for a second confirmation. Surface only the created-record summary or final blockers."
        ),
