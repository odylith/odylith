...
def _launcher_shell_common_lines(*, fallback_source_root: Path | None) -> list[str]:
    ...
    return [
        ...
        "  sed -n 's#^[[:space:]]*exec \"\\([^\"]*\\)\".*#\\1#p' \"$python_path\" | head -n 1",
        "}",
        "odylith_wrapper_source_root() {",
        '  local python_path="${1:-}"',
        '  [[ -f "$python_path" ]] || return 1',
        "  sed -n 's#^[[:space:]]*export PYTHONPATH=\"\\([^$\"]*\\).*#\\1#p' \"$python_path\" | head -n 1",
        "}",
        ...
        *managed_runtime_launcher_verifier_lines(),
        ...
        '    odylith_managed_runtime_launch_ready "$current_root" "$current_python" && return 0',
        '    odylith_legacy_managed_runtime_compatible "$current_root" && return 0',
        "    return 1",
        "  fi",
        ...
        "    return 0",
        "  fi",
        '  if [[ "$fallback_python" == "$state_root/runtime/versions/"*"/bin/"* ]]; then',
        '    fallback_root="$(odylith_runtime_root_for_python "$fallback_python")"',
    ...
    return "\n".join(
        [
            ...
            *_launcher_shell_common_lines(fallback_source_root=resolved_source_root),
            ...
            "  else",
            '    install_url="https://github.com/freedom-research/odylith/releases/latest/download/install.sh"',
            "  fi",
            ...
            *([export_line] if export_line else []),
            ...
            "  fi",
            "fi",
            'echo "Odylith launcher detected untrusted or unhealthy runtime state. From the repo root, try `./.odylith/bin/odylith-bootstrap doctor --repo-root . --repair`." >&2',
            "exit 1",
            "",
        ]
    )
    ...
    return "\n".join(
        [
            ...
            *([export_line] if export_line else []),
            ...
            "  done",
            "fi",
            'echo "Odylith bootstrap could not find a trusted repo-local runtime. From the repo root, try `./.odylith/bin/odylith-bootstrap doctor --repo-root . --repair`; rerun the hosted installer only if no trusted repo-local runtime is left." >&2',
            "exit 1",
            "",
        ]
    )
