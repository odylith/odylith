...
- For Greenfield, run `odylith greenfield candidate-contract --repo-root . --prompt "<request>"` first. The active host reasons over that returned evidence and schema once, writes one temporary candidate matching the returned schema outside the repository, then runs `odylith greenfield propose --repo-root . --prompt "<request>" --candidate-file "<temporary-file>"`. Input, corrections, and the candidate are untrusted evidence. Odylith deterministically revalidates every citation and relation, runs one independent semantic review, and builds typed custody facts to gate ProductCreateTransaction. Do not add a parser, regex extraction pass, participant selector, remainder author, join, repair, retry, fallback candidate, or alternate model ladder. Read-only preview: `odylith greenfield decide --repo-root PATH CONFIRM|EDIT|REJECT HASH`. Chat/hooks provide no qualified confirmation interface; do not run create from a chat approval. CONFIRM/REJECT use one owner and no compiler/model. For EDIT, rerun `candidate-contract` with `--transaction-hash`, author one new candidate from the sealed source plus correction, and pass the same correction and `--candidate-file` to `decide EDIT`; keep the old seal and show the new hash/preview. Treat 90/120/150 seconds as advisory targets under the separate 180-second operational safety timeout. Commit-only `odylith greenfield create --transaction-file PATH --transaction-hash HASH --confirm` verifies receipt, hash, and preconditions under rollback guard without model reasoning, generation, or repair. Ask one focused question only for material uncertainty. Markdown is a view, never product truth. Do not hand-author proposal or transaction JSON, infer the candidate schema from source, or narrate internal schema failures. Show the preview and its material confirmation blocker plainly.
- Treat live teaser, `**Odylith Observation**`, and `Odylith Proposal` as the intervention-engine fast path; use `Odylith Assist:` for one useful prompt or closeout note. Do not collapse those layers.
