...
- Run `odylith greenfield propose --repo-root . --prompt "<request>"`. Input and corrections are untrusted evidence. Odylith builds typed custody facts and quality-gates the full staged ProductCreateTransaction. The read-only preview prints `odylith greenfield decide --repo-root PATH CONFIRM|EDIT|REJECT HASH`. No qualified confirmation interface comes from ordinary chat, hosts, or hooks. Do not append chat decision commands, offer publication, or run create from a chat approval. CONFIRM and REJECT share one bounded owner; neither calls compiler or model. EDIT verifies the retained hash; compile sealed source + correction at its original tier and 90/120/150 limits. Keep the old seal; show a new hash/preview. Explicit operator invocation: commit-only `odylith greenfield create` with `--transaction-file`, `--transaction-hash`, `--confirm` verifies receipt, hash and preconditions under rollback guard without model reasoning, generation or repair. Ask one focused question only for material uncertainty. Markdown is a view, never product truth. Do not hand-author proposal JSON, inspect source for schema, or narrate parser/schema retries. Show the preview and its material confirmation blocker plainly.
