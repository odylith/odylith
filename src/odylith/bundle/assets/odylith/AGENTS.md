...
- Empty or thin consumer repos can still receive proposal-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt "<request>"`; render backlog, program waves, release plan, planned Registry components, draft Atlas diagrams, assumptions, risks, and validation while preserving observed-source versus user-intent evidence, and write only after confirmation.
