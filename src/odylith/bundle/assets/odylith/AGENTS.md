...
- Run `odylith greenfield propose --repo-root . --prompt "<request>"`. Input and corrections are untrusted evidence. Odylith builds typed custody facts to gate ProductCreateTransaction. Read-only preview: `odylith greenfield decide --repo-root PATH CONFIRM|EDIT|REJECT HASH`. Chat/hooks provide no qualified confirmation interface. Do not append chat decision commands, offer publication, or run create from a chat approval. CONFIRM/REJECT: one owner, no compiler/model. EDIT verifies retained hash and compiles sealed source + correction at original tier to 90/120/150-second performance targets under a separate 180-second operational safety timeout. Report target overruns as timing evidence, not failures alone. Keep old seal; show new hash/preview. Commit-only `odylith greenfield create` takes `--transaction-file`, `--transaction-hash`, `--confirm`; it verifies receipt/hash/preconditions under rollback guard without model reasoning, generation or repair. Ask one focused question only for material uncertainty. Markdown is a view, never product truth. Do not hand-author proposal JSON, inspect source for schema, or narrate parser/schema retries. Show the preview and its material confirmation blocker plainly.
