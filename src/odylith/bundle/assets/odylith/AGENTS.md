...
- Empty or thin consumer repos can still receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt "<request>"` to review the canonical apply-ready host-reasoning evidence/schema contract, including direction options, coding-readiness gates, and host-authored Atlas Mermaid source. On confirmation, use `odylith greenfield create --repo-root . --prompt "<request>" --release 0.0.1 --confirm` so Odylith owns proposal construction, validation, Tribunal, governed writes, refresh, and handoff. Do not hand-author proposal JSON, reconstruct it from prose, or start coding before the project-first gates are accepted.
