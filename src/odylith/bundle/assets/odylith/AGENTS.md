...
- Empty or thin consumer repos can still receive project-first governance from user intent. Use `odylith greenfield propose --repo-root . --prompt "<request>"`; it treats the request and optional EDIT Markdown as untrusted evidence, builds typed custody facts, repairs and quality-gates the full staged ProductCreateTransaction, and then renders the sole visible confirmation view. The preview uses short sections for Product story, State object, First complete path, Human actors, External systems, Internal product systems, assumptions, ambiguities, and proof boundary, followed by one clear `## Choose one command` block. **CONFIRM** commits the exact shown transaction hash with `CONFIRM <hash>`; **EDIT** supplies corrections as new evidence with `EDIT <hash> <corrections>` and rebuilds the typed facts, package, and hash; **REJECT** stops with `REJECT <hash>` and writes no records. Only material uncertainty warrants one focused question; non-material gaps become visible assumptions. Markdown is evidence and a human view, never product truth. `odylith greenfield create --repo-root . --transaction-file .odylith/runtime/greenfield/pending/<hash>/product-create-transaction.v1.json --transaction-hash <hash> --confirm` only verifies the receipt, hash, and repo preconditions, writes sealed bytes under rollback guard, and validates readback. It does not parse product Markdown, call a host model, generate artifacts, or repair prose after CONFIRM. Do not inspect source, hand-author/repair proposal JSON, narrate parser/schema retries, or request a second confirmation. Surface only the final transaction, created-record summary, or a material blocker.
