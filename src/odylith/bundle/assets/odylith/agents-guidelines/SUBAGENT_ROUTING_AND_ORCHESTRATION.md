...
- In Claude Code, execute delegated leaves through the emitted Task-tool payload plus the checked-in `.claude/` project assets. In other non-Codex runtimes, keep the emitted plan as local guidance unless the host contract explicitly proves native delegation.
...
- These defaults apply only when the current host supports native delegation. Codex uses routed `spawn_agent` payloads, while Claude Code uses Task-tool subagents under the same bounded contract.
