...
run `odylith start` first, wait for it to finish, and only then run
`odylith context <ref>` when a precise anchor is known. Widen to direct `rg`,
source reads, or standalone host search only when Odylith reports ambiguity,
missing anchors, repair conditions, or explicit fallback.
