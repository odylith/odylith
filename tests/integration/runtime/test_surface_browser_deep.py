        ...
        for _pw, browser in _browser():
            ...
            try:
                with _new_page(context) as (page, observation):
                    ...
                    assert not [
                        failure for failure in snapshot.native_failures
                        if failure.url.endswith(f"/odylith/compass/runtime/history/{archived_day}.v1.json")
                    ]
                    assert not [
                        error for error in snapshot.http_errors
                        if error.url.endswith(f"/odylith/compass/runtime/history/{archived_day}.v1.json")
                    ]
                    _assert_clean_page(page, observation)
            finally:
                context.close()
        ...
        for _pw, browser in _browser():
            ...
            try:
                with _new_page(context) as (page, observation):
                    ...
                    expected_resource = urlsplit(base_url + "/odylith/compass/compass-source-truth.v1.json")
                    snapshot = observation.finish()
