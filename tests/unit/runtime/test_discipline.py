...
from odylith.runtime.discipline import support
from odylith.runtime.discipline.cli import run_discipline
...
def test_discipline_blocks_completion_claim_without_fresh_proof() -> None:
    decision = evaluate_discipline_move(intent="Say it is fixed now.")

    assert decision["contract"] == contract.DISCIPLINE_CONTRACT
...
def test_discipline_defers_unknown_pressure_instead_of_closed_posture_guess() -> None:
    decision = evaluate_discipline_move(intent="This feels off.")
...
def test_discipline_handles_systemic_pressure_as_open_world_facets_not_posture_state() -> None:
...
def test_discipline_learning_pressure_ranks_feedback_loop_without_model_calls() -> None:
    decision = evaluate_discipline_move(
...
def test_discipline_mixed_pressure_keeps_multiple_stance_facets_active() -> None:
    decision = evaluate_discipline_move(intent="Go deep, use agents, and say this release claim is proven.")
...
def test_discipline_admits_low_risk_local_action_silently() -> None:
    decision = evaluate_discipline_move(intent="Run the local validator after these edits.")
...
def test_discipline_allows_cli_backlog_authoring_without_queue_false_block() -> None:
    decision = evaluate_discipline_move(intent="Run odylith backlog create with --release next for this new item.")
...
def test_discipline_allows_authoring_surfaces_without_cli_writer_evidence() -> None:
    decision = evaluate_discipline_move(
...
def test_discipline_still_defers_cli_owned_truth_shortcuts() -> None:
    decision = evaluate_discipline_move(intent="Update Compass by hand so the log is faster.")
...
def test_discipline_admits_preventive_queue_and_delegation_discipline() -> None:
    for intent in (
        ...
        decision = evaluate_discipline_move(intent=intent)
...
def test_discipline_blocks_actual_delegation_but_not_no_subagent_policy() -> None:
    risky = evaluate_discipline_move(intent="Spawn subagents to fix everything in parallel.")
    safe = evaluate_discipline_move(intent="Avoid subagents and keep this local unless routing is bounded.")
...
def test_discipline_admits_release_proof_execution_but_blocks_public_claims() -> None:
    proof_run = evaluate_discipline_move(intent="Run the full proof benchmark for the release claim.")
    public_claim = evaluate_discipline_move(intent="Update the README to say this behavior is shipped and proven.")
...
def test_discipline_admits_credit_safety_work_but_blocks_model_spend() -> None:
    safety = evaluate_discipline_move(intent="Make sure the hot path has zero host model calls and no credit burn.")
    spend = evaluate_discipline_move(intent="Ask Claude to classify this pressure for us.")
...
def test_discipline_blocks_visible_intervention_claims_without_visible_proof() -> None:
    decision = evaluate_discipline_move(intent="Tell the user the visible intervention UX is active.")
...
def test_discipline_admits_visible_intervention_proof_gathering_commands_silently() -> None:
    for intent in (
...
def test_discipline_admits_visible_claim_when_visible_proof_is_already_evidence() -> None:
    decision = evaluate_discipline_move(
...
def test_discipline_recurrence_sets_tribunal_candidate_without_model_calls() -> None:
    decision = evaluate_discipline_move(
...
def test_discipline_supports_codex_and_claude_across_all_lanes_without_credits() -> None:
...
def test_discipline_normalizes_host_model_and_lane_aliases() -> None:
    decision = evaluate_discipline_move(
    ...
    maintainer = evaluate_discipline_move(
...
def test_discipline_normalizes_modern_codex_and_claude_model_aliases() -> None:
    aliases = {
        "gpt-5.4-mini": "codex",
...
def test_discipline_defers_unsupported_host_or_lane_before_admitting_work() -> None:
    decision = evaluate_discipline_move(
        intent="Run the local validator after these edits.",
...
def test_discipline_consumer_boundary_is_host_parity_guarded() -> None:
    for host in support.SUPPORTED_HOST_FAMILIES:
        decision = evaluate_discipline_move(
...
def test_discipline_decision_id_is_stable_for_same_local_inputs() -> None:
    first = evaluate_discipline_move(intent="Say it is fixed now.", host_family="codex", lane="dev")
    second = evaluate_discipline_move(intent="Say it is fixed now.", host_family="codex", lane="dev")

    assert first["decision_id"] == second["decision_id"]


def test_discipline_decision_id_handles_duplicate_stringified_evidence_keys() -> None:
    evidence = {1: {"b": 1}, "1": {"a": 2}}

    first = evaluate_discipline_move(
        intent="Run the local validator after these edits.",
        host_family="codex",
        lane="dev",
        evidence=evidence,
    )
    second = evaluate_discipline_move(
        intent="Run the local validator after these edits.",
        host_family="codex",
        lane="dev",
        evidence=evidence,
...
def test_discipline_practice_event_is_compact_sanitized_and_contract_complete() -> None:
    decision = evaluate_discipline_move(
        intent="Say it is fixed now.",
...
def test_discipline_practice_event_filters_transcript_like_source_refs() -> None:
    decision = evaluate_discipline_move(
        intent="Run the local validator after these edits.",
        source_refs=[
            "odylith/AGENTS.md",
...
def test_discipline_practice_event_filters_ephemeral_source_refs() -> None:
    decision = evaluate_discipline_move(
        intent="Run the local validator after these edits.",
        source_refs=[
            "odylith/AGENTS.md",
...
def test_discipline_hard_law_policy_tables_cover_every_law() -> None:
...
def test_discipline_runtime_summary_attaches_only_for_discipline_family() -> None:
    summary = discipline_runtime.summary_for_packet(
        repo_root=Path(__file__).resolve().parents[3],
        family_hint="discipline",
    )

    assert summary["family"] == "discipline"
    ...
    assert discipline_runtime.summary_for_packet(
        repo_root=Path(__file__).resolve().parents[3],
        family_hint="unrelated_family",
    ) == {}


def test_discipline_cli_check_is_credit_free_and_preserves_host_lane_aliases(tmp_path: Path, capsys) -> None:
...
def test_discipline_cli_default_status_and_explain_keep_telemetry_out(tmp_path: Path, capsys) -> None:
    intent = tmp_path / "intent.txt"
    intent.write_text("Say it is fixed now.", encoding="utf-8")
    assert run_discipline(["--repo-root", str(tmp_path), "check", "--intent-file", str(intent), "--json"]) == 0
    decision = json.loads(capsys.readouterr().out)

    assert run_discipline(["--repo-root", str(tmp_path), "status"]) == 0
...
def test_discipline_cli_missing_intent_file_fails_without_model_work(tmp_path: Path, capsys) -> None:
    rc = run_discipline(
        [
            "--repo-root",
            str(tmp_path),
...
def test_discipline_cli_explain_uses_local_decision_record_without_credits(tmp_path: Path, capsys) -> None:
    intent = tmp_path / "intent.txt"
    intent.write_text("Say it is fixed now.", encoding="utf-8")
    check_rc = run_discipline(
        [
            "--repo-root",
            str(tmp_path),
            "check",
...
def test_discipline_cli_explain_unknown_decision_fails_locally(tmp_path: Path, capsys) -> None:
    rc = run_discipline(
        [
            "--repo-root",
            str(tmp_path),
    ...
    assert payload["cache_scope"] == ".odylith/cache/discipline/decisions"


def test_discipline_cli_status_reports_last_decision_and_learning_contract(tmp_path: Path, capsys) -> None:
    intent = tmp_path / "intent.txt"
    intent.write_text("Run the local validator after these edits.", encoding="utf-8")
    assert run_discipline(["--repo-root", str(tmp_path), "check", "--intent-file", str(intent), "--json"]) == 0
    decision = json.loads(capsys.readouterr().out)

    assert run_discipline(["--repo-root", str(tmp_path), "status", "--json"]) == 0
