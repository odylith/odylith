...
def test_post_confirm_issue_classifier_fails_closed_for_legacy_untyped_issues() -> None:
    ...
    assert issue.surface == "preconfirm"
    assert issue.path == ""
...
def test_post_confirm_issue_classifier_preserves_unmatched_legacy_blockers_with_findings() -> None:
    ...
    issues = engine.classify_greenfield_preconfirm_issues(report)
...
def test_completion_report_serializes_typed_review_report() -> None:
    ...
    assert payload["review_report"]["version"] == "odylith.greenfield.preconfirm.review_report.v1"
...
def test_preconfirm_engine_stops_on_repeated_failure_signature(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ) -> engine.GreenfieldPreconfirmRepairContext:
        operation = {
...
def test_preconfirm_engine_rejects_generated_copy_quality_as_completion_priority_debt(
...
def test_preconfirm_engine_commits_typed_package_repetition_projection_debt(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    package = GreenfieldCompletionPackage(proposal={}, release_selector="0.0.1")
    message = (
    ...
    assert manifest["status"] == engine.PRECONFIRM_COMPLETION_PRIORITY_STATUS
...
def test_preconfirm_engine_rejects_source_truth_package_repetition_debt(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repeated = (
...
def test_preconfirm_engine_rejects_untyped_package_repetition_debt(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    package = GreenfieldCompletionPackage(proposal={}, release_selector="0.0.1")
...
def test_preconfirm_engine_passes_quality_lens_context_to_semantic_repair(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    reports = [
        GreenfieldCompletionReport(
    ...
    repair_contexts: list[engine.GreenfieldPreconfirmRepairContext] = []
    ...
    assert context.review_report["version"] == "odylith.greenfield.preconfirm.review_report.v1"
    assert context.patchset_request["version"] == "odylith.greenfield.preconfirm.patchset_request.v1"
...
def test_patchset_semantic_coverage_operation_without_structured_fact_does_not_synthesize_first_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert repaired["intent"]["first_path"] == (
        "A shelter capacity coordination workspace for municipal emergency operations teams."
    )
    assert "preconfirm_patch_application_ledger" not in repaired
    assert "semantic_model" not in repaired or "first_path_contract" not in repaired["semantic_model"]
...
def test_preconfirm_engine_uses_scoped_prewrite_rerender_after_patch_ledger(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def fake_repair_proposal(
        ...
        repaired["preconfirm_patch_application_ledger"] = [
            {
...
def test_greenfield_apply_result_carries_commit_manifest(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert manifest["version"] == engine.PRECONFIRM_QUALITY_MANIFEST_VERSION
    assert manifest["engine"] == engine.PRECONFIRM_ENGINE_VERSION
    ...
    assert manifest["patchset_request"]["version"] == "odylith.greenfield.preconfirm.patchset_request.v1"
