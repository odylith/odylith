"""One complete-author call: full pinned budget, exact custody, and no hidden repair."""
...
def test_one_complete_author_uses_full_pinned_window(profile_id, model_budget, consumer_budget):
    response = _response(_source())
    clock = Clock()
    provider = Provider([response], [model_budget], clock)
    ...
    assert result.semantic_model_call_count == len(provider.requests) == 1
...
def test_overrun_fails_after_one_call_without_review_or_retry(profile_id):
    ...
    with pytest.raises(author.GreenfieldModelAuthoringError, match="declared time window"):
...
def test_invalid_candidate_never_reaches_a_second_prepared_response_or_writes(tmp_path, failure):
    ...
    with pytest.raises(author.GreenfieldModelAuthoringError):
        materialize_model_authored_intent(prompt=source, repo_root=tmp_path, authoring_provider=provider)
    assert len(provider.requests) == 1
...
def test_source_actor_custody_cannot_be_malformed_or_remove_a_referenced_human(tmp_path, human_actors):
    ...
    with pytest.raises(author.GreenfieldModelAuthoringError):
        materialize_model_authored_intent(prompt=_source(), repo_root=tmp_path, authoring_provider=provider)
...
def test_model_observation_is_checked_before_and_after_the_only_call(monkeypatch):
    ...
    author.author_greenfield_intent(evidence_text=_source(), provider=provider, clock=lambda: 0.0)
    assert provider.calls == 1
...
def test_proof_preserves_the_exact_only_candidate_without_review_metadata(tmp_path, monkeypatch, failed):
    response = _response(_source())
    if failed:
...
def test_source_binding_preserves_explicit_unicode_occurrences_without_auto_repair():
    ...
    provider = StructuredAuthoringProvider(response)
    result = author.author_greenfield_intent(evidence_text=source, provider=provider, clock=lambda: 0.0)
