...
def test_confirmed_intent_semantic_terms_stay_in_text_owner() -> None:
    text_source = CONFIRMED_TEXT_PATH.read_text(encoding="utf-8")
    validation_source = (DOMAIN_INTELLIGENCE / "greenfield_confirmed_intent_validation.py").read_text(encoding="utf-8")
    system_rows_source = (DOMAIN_INTELLIGENCE / "greenfield_confirmed_system_rows.py").read_text(encoding="utf-8")
