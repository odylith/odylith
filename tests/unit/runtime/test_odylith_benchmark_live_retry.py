...
def test_run_live_scenario_retries_negative_exit_missing_schema_without_writes(
    ...
    assert result["live_execution"]["infra_retry_reason"] == "negative_host_exit_missing_schema_output"
    assert result["live_execution"]["infra_retry_replaced_exit_code"] == -15


def test_run_live_scenario_retries_transient_capacity_missing_schema(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    attempts: list[Mapping[str, Any]] = [
        _result(
            ...
            stderr_tail="Selected model is at capacity. Please try a different model.",
        ),
        _result(exit_code=0, validation_summary="passed", expectation_ok=True),
    ]
    ...
    assert attempts == []
    assert result["live_execution"]["infra_retry_attempts"] == 1
    assert result["live_execution"]["infra_retry_reason"] == "transient_host_capacity_missing_schema_output"
    assert result["live_execution"]["infra_retry_replaced_exit_code"] == 1


def test_run_live_scenario_retries_transient_infra_twice(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    attempts: list[Mapping[str, Any]] = [
        ...
        _result(
            exit_code=1,
            validation_summary="missing_schema_output",
            stdout_tail="Selected model is at capacity. Please try a different model.",
        ),
        _result(exit_code=0, validation_summary="passed", expectation_ok=True),
    ]
    ...
    assert result["expectation_ok"] is True
    assert attempts == []
    assert result["live_execution"]["infra_retry_attempts"] == 2
    assert result["live_execution"]["infra_retry_reasons"] == [
        "negative_host_exit_missing_schema_output",
        "transient_host_capacity_missing_schema_output",
    ]
