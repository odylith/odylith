...
def _authored_state(repo: Path, *, extra: tuple[str, ...] = ()) -> dict[str, tuple[bytes, int]]:
    return {
        name: ((repo / name).read_bytes(), (repo / name).stat().st_mode)
        for name in (INDEX, PLAN, RADAR, IDEA, *extra)
    }
...
def _add_shared_execution_bindings(repo: Path, *, reverse: bool) -> tuple[str, ...]:
    shared_plan = PLAN.replace("assessment.md", "shared.md")
    (repo / shared_plan).write_text(METADATA.replace("B-202", "B-210"), encoding="utf-8")
    shared_ideas = tuple(f"{IDEAS}/2026-09-09-shared-{idea_id.lower()}.md" for idea_id in ("B-210", "B-211"))
    plan_rows, radar_rows = [], []
    for idea_id, idea_path in zip(("B-210", "B-211"), shared_ideas, strict=True):
        (repo / idea_path).write_text(_idea_text(
            idea_id=idea_id, title=f"Shared assessment {idea_id}", date="2026-09-09",
            status="implementation", promoted_to_plan=shared_plan,
        ), encoding="utf-8")
        plan_rows.append(f"| `{shared_plan}` | In progress | 2026-07-20 | 2026-09-09 | `{idea_id}` |\n")
        radar_rows.append(f"| - | {idea_id} | Shared assessment {idea_id} | P1 | 90 | 4 | 4 | 4 | M | Medium | implementation | [idea]({idea_path}) |\n")
    index = repo / INDEX
    index.write_text(index.read_text(encoding="utf-8") + "".join(reversed(plan_rows) if reverse else plan_rows), encoding="utf-8")
    radar = repo / RADAR
    snapshot = backlog.load_backlog_index_snapshot(radar)
    lines = radar.read_text(encoding="utf-8").splitlines(keepends=True)
    start, _end = reconcile._find_section_bounds(lines, "## " + snapshot["execution"]["section_title"])
    lines[start + 4:start + 4] = radar_rows
    radar.write_text("".join(lines), encoding="utf-8")
    assert backlog._validate_idea_specs(repo / IDEAS)[1] == []
    return shared_plan, *shared_ideas


@pytest.mark.parametrize("reverse", [False, True])
def test_shared_execution_noops_do_not_block_unique_repair_or_reach_writers(
    repo: Path, reverse: bool, monkeypatch: pytest.MonkeyPatch,
) -> None:
    old_row = _stale_execution_row(repo)
    shared = _add_shared_execution_bindings(repo, reverse=reverse)
    before = _authored_state(repo, extra=shared)
    written_ids = []
    original_move = reconcile._move_active_row_to_execution_status

    def track_move(*args, **kwargs):
        written_ids.append(kwargs["idea_id"])
        return original_move(*args, **kwargs)

    monkeypatch.setattr(reconcile, "_move_active_row_to_execution_status", track_move)
    decisions, successors = _run(repo, plans=(PLAN, shared[0]))

    assert [(row.action, row.backlog_after) for row in decisions] == [("repair_execution_section_status", "B-202")]
    assert successors == [] and written_ids == ["B-202"]
    after = _authored_state(repo, extra=shared)
    new_row = old_row.replace("| finished |", "| implementation |")
    assert after[RADAR][0].replace((new_row + "\n").encode(), b"", 1) == before[RADAR][0].replace((old_row + "\n").encode(), b"", 1)
    assert after[RADAR][1] == before[RADAR][1]
    assert {key: value for key, value in after.items() if key != RADAR} == {
        key: value for key, value in before.items() if key != RADAR
    }
    assert _run(repo, plans=(PLAN, shared[0])) == ([], [])
    assert written_ids == ["B-202"]
    assert _authored_state(repo, extra=shared) == after


@pytest.mark.parametrize("reverse", [False, True])
@pytest.mark.parametrize("member", [0, 1])
@pytest.mark.parametrize("damage", ["stale_status", "misplaced", "queued", "nonreciprocal"])
def test_shared_execution_requires_every_binding_healthy_before_any_write(
    repo: Path, reverse: bool, member: int, damage: str,
) -> None:
    _stale_execution_row(repo)
    shared = _add_shared_execution_bindings(repo, reverse=reverse)
    idea_id = ("B-210", "B-211")[member]
    if damage in {"stale_status", "misplaced"}:
        radar = repo / RADAR
        content = radar.read_text(encoding="utf-8")
        row = next(line for line in content.splitlines(keepends=True) if f"| {idea_id} |" in line)
        if damage == "stale_status":
            content = content.replace(row, row.replace("| implementation |", "| finished |"))
        else:
            content = content.replace(row, "")
            target = next(line for line in content.splitlines(keepends=True) if "| B-202 |" in line)
            content = content.replace(target, target + row)
        radar.write_text(content, encoding="utf-8")
    else:
        idea = repo / shared[member + 1]
        old, new = (("status: implementation", "status: queued") if damage == "queued" else
                    (f"promoted_to_plan: {shared[0]}", "promoted_to_plan: other.md"))
        idea.write_text(idea.read_text(encoding="utf-8").replace(old, new), encoding="utf-8")
    before = _authored_state(repo, extra=shared)

    with pytest.raises(ValueError, match="unique indexed|reciprocal"):
        _run(repo, plans=(PLAN, shared[0]))

    assert _authored_state(repo, extra=shared) == before


def test_shared_execution_noop_rejects_duplicate_workstream_identity(repo: Path) -> None:
    _stale_execution_row(repo)
    shared = _add_shared_execution_bindings(repo, reverse=False)
    index = repo / INDEX
    content = index.read_text(encoding="utf-8")
    row = next(line for line in content.splitlines(keepends=True) if "`B-210`" in line)
...
@pytest.mark.parametrize("reverse", [False, True])
@pytest.mark.parametrize("damage", [
    "extra_cell", "second_table", "noncontiguous_table", "bad_separator", "symlink", "hardlink",
])
def test_shared_noop_requires_complete_active_index_layout_before_repair(
    repo: Path, reverse: bool, damage: str,
) -> None:
    _stale_execution_row(repo)
    shared = _add_shared_execution_bindings(repo, reverse=reverse)
    index = repo / INDEX
    content = index.read_text(encoding="utf-8")
    duplicate = next(line for line in content.splitlines() if "`B-210`" in line)
    header = "| Plan | Status | Created | Updated | Backlog |\n| --- | --- | --- | --- | --- |\n"
    if damage == "extra_cell":
        index.write_text(content + duplicate + " extra |\n", encoding="utf-8")
    elif damage in {"second_table", "noncontiguous_table"}:
        heading = "\n## Active Plans\n\n" if damage == "second_table" else "\nAn authored note.\n\n"
        index.write_text(content + heading + header + duplicate + "\n", encoding="utf-8")
    elif damage == "bad_separator":
        index.write_text(content.replace("| --- | --- | --- | --- | --- |", "| invalid | --- | --- | --- | --- |"), encoding="utf-8")
    else:
        target = index.with_name("unrelated-index.md")
        if damage == "hardlink":
            target.hardlink_to(index)
    ...
    with pytest.raises(ValueError, match="Active Plans|index alias|unique indexed"):
        _run(repo, plans=(PLAN, shared[0]))

    assert _authored_state(repo, extra=shared) == before


@pytest.mark.parametrize(("backlog_id", "action"), [
    ("-", "missing_backlog_binding"),
    ("invalid", "invalid_backlog_binding"),
    ("B-999", "missing_workstream"),
])
def test_existing_single_binding_diagnostics_remain_read_only(
    repo: Path, backlog_id: str, action: str,
) -> None:
    _run(repo)
    index = repo / INDEX
    index.write_text(index.read_text(encoding="utf-8").replace("`B-202`", f"`{backlog_id}`"), encoding="utf-8")
    before = _authored_state(repo)

    decisions, successors = _run(repo)

    assert [row.action for row in decisions] == [action]
    assert successors == []
    assert _authored_state(repo) == before
