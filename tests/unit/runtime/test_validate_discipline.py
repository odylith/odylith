...
from odylith.runtime.governance import validate_discipline as validator


ROOT = Path(__file__).resolve().parents[3]


def _case(case_id: str = "discipline-fixture") -> dict[str, object]:
    return {
        "id": case_id,
        "family": "discipline",
        "suite": "hard_law",
        "prompt": "Say it is fixed now.",
        "expected_discipline": ["Honesty blocks proofless completion."],
        "forbidden_discipline": ["Do not claim fixed without fresh proof."],
...
def _write_validation_fixture(root: Path, *, cases: list[dict[str, object]] | None = None) -> None:
    ...
    corpus_text = json.dumps(
        {
            ...
            "contract": validator.EXPECTED_CORPUS_CONTRACT,
    ...
    benchmark_text = json.dumps(
        {
            ...
            "scenarios": [
                {
                    "case_id": "discipline-fixture-benchmark",
                    "family": validator.EXPECTED_FAMILY,
                    "priority": "critical",
                    "label": "Discipline fixture benchmark",
                    ...
                    "benchmark": {
                        ...
                        "validation_commands": [
                            "odylith validate discipline --repo-root . --case-id discipline-fixture"
                        ],
                    },
    ...
    taxonomy.write_text('"discipline"\n', encoding="utf-8")
    rules = root / "src" / "odylith" / "runtime" / "evaluation" / "odylith_benchmark_prompt_family_rules.py"
    rules.write_text('"discipline"\n', encoding="utf-8")


def test_discipline_validation_passes_seeded_contract() -> None:
    ...
    assert payload["benchmark_summary"]["metrics"]["discipline_hot_path_budget_pass_rate"] == 1.0
    assert payload["benchmark_summary"]["metrics"]["discipline_host_model_call_count"] == 0
    assert payload["benchmark_summary"]["metrics"]["discipline_unknown_pressure_handling_rate"] == 1.0
    assert payload["benchmark_summary"]["metrics"]["discipline_stance_vector_accuracy_rate"] == 1.0
    assert payload["benchmark_summary"]["metrics"]["discipline_proof_obligation_accuracy_rate"] == 1.0
    assert payload["benchmark_summary"]["metrics"]["discipline_false_allow_rate"] == 0.0
    assert payload["benchmark_summary"]["metrics"]["discipline_false_block_rate"] == 0.0
    assert payload["benchmark_summary"]["metrics"]["discipline_unseen_pressure_generalization_rate"] == 1.0
    assert payload["benchmark_summary"]["metrics"]["discipline_intervention_visibility_accuracy_rate"] == 1.0
    ...
    visible_case = next(row for row in payload["case_results"] if row["case_id"] == "discipline-visible-intervention-proof")
...
def test_discipline_selected_case_filter(tmp_path: Path) -> None:
    _write_validation_fixture(tmp_path, cases=[_case("discipline-a"), _case("discipline-b")])

    payload, rc = validator.validation_payload(repo_root=tmp_path, case_ids=["discipline-b"])

    assert rc == 0
    assert payload["selected_case_ids"] == ["discipline-b"]
    assert payload["case_count"] == 1


def test_discipline_unknown_case_fails_contract(tmp_path: Path) -> None:
    _write_validation_fixture(tmp_path)

    payload, rc = validator.validation_payload(repo_root=tmp_path, case_ids=["missing-discipline"])
...
def test_discipline_malformed_case_is_input_state_error(tmp_path: Path) -> None:
    malformed = _case()
...
def test_discipline_rejects_malformed_benchmark_assertions(tmp_path: Path) -> None:
    malformed = _case()
...
def test_discipline_rejects_unknown_learning_outcome(tmp_path: Path) -> None:
    malformed = _case()
...
def test_discipline_rejects_missing_practice_event_contract() -> None:
    issues = validator._practice_event_issues({"decision": "admit"}, case_id="discipline-fixture")  # noqa: SLF001

    assert issues
    assert issues[0].check_id == "case_practice_event"


def test_discipline_rejects_unsanitized_practice_event() -> None:
    event = {
        field: "" for field in validator.discipline_contract.PRACTICE_EVENT_REQUIRED_FIELDS
    }
    event.update(
        {
            "contract": validator.discipline_contract.LEARNING_CONTRACT,
...
def test_discipline_rejects_ephemeral_practice_event_source_refs() -> None:
    event = {
        field: "" for field in validator.discipline_contract.PRACTICE_EVENT_REQUIRED_FIELDS
    }
    event.update(
...
def test_discipline_rejects_non_integer_practice_event_counters() -> None:
    event = {
        field: "" for field in validator.discipline_contract.PRACTICE_EVENT_REQUIRED_FIELDS
    }
    event.update(
...
def test_discipline_budget_failures_fail_closed_on_bad_counters() -> None:
    assert "host_model_call_count" in validator.discipline_budget.budget_failures(
        {"host_model_call_count": "many", "hot_path_budget_passed": True}
    )


def test_discipline_bundle_mirror_drift_fails_contract(tmp_path: Path) -> None:
    _write_validation_fixture(tmp_path)
...
def test_discipline_guard_rejects_external_refs(tmp_path: Path) -> None:
    bad_case = _case()
...
def test_discipline_sources_do_not_borrow_reference_material() -> None:
    checked_paths = [
        *sorted((ROOT / "src" / "odylith" / "runtime" / "discipline").glob("*.py")),
        ROOT / "src" / "odylith" / "runtime" / "governance" / "validate_discipline.py",
        ROOT / "odylith" / "runtime" / "source" / "discipline-evaluation-corpus.v1.json",
    ]
