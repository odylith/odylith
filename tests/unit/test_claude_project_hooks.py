...
def test_show_me_prompt_guard_routes_capability_inventory_to_product_command(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    ...
    completed = _run_hook(
        "show-me-prompt-guard.py",
        repo_root,
        payload={"prompt": "List all the Odylith capabilities and engines"},
    )
    ...
    additional_context = payload["hookSpecificOutput"]["additionalContext"]
    assert payload["hookSpecificOutput"]["hookEventName"] == "UserPromptSubmit"
    assert "Odylith capability-inventory route lock" in additional_context
